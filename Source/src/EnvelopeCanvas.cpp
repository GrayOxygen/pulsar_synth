...
void EnvelopeCanvas::setEnvelopeData(const std::array<float, ENVELOPE_SIZE>& data) {
    envelopeData = data;
    repaint();
    sendChangeMessage();
}

std::array<float, ENVELOPE_SIZE> EnvelopeCanvas::getEnvelopeData() const {
    return envelopeData;
}
...
float EnvelopeCanvas::getEnvelopeValueAtPhase(float phase) const {
    ...
    float indexF = phase * (ENVELOPE_SIZE - 1);
    int index = static_cast<int>(indexF);
    float frac = indexF - index;

    // 线性插值
    float val1 = envelopeData[std::min(index, ENVELOPE_SIZE - 1)];
    float val2 = envelopeData[std::min(index + 1, ENVELOPE_SIZE - 1)];
...
int EnvelopeCanvas::phaseToX(float phase) const {
    return static_cast<int>(phase * getWidth());
}
...
void EnvelopeCanvas::drawGrid(juce::Graphics& g) {
    ...
    // 垂直线 (每256个点一条)
    for (int i = 0; i < ENVELOPE_SIZE; i += 256) {
        float x = static_cast<float>(i) / ENVELOPE_SIZE * getWidth();
...
void EnvelopeCanvas::drawEnvelope(juce::Graphics& g) {
    ...
    for (int i = 0; i < ENVELOPE_SIZE; ++i) {
        float x = static_cast<float>(i) / (ENVELOPE_SIZE - 1) * getWidth();
...
void EnvelopeCanvas::updateEnvelopeFromMouse(const juce::MouseEvent& event) {
    ...
    // 计算影响的索引范围（简单的圆形笔刷）
    int centerIndex = static_cast<int>(phase * (ENVELOPE_SIZE - 1));
    int brushRadius = 10; // 影响半径

    for (int i = -brushRadius; i <= brushRadius; ++i) {
        int idx = centerIndex + i;
        if (idx >= 0 && idx < ENVELOPE_SIZE) {
            // 距离衰减
