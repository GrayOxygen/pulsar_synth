...
void PulsarSynthVoice::spawnGrain() {
  ...
  double waveformFreq = snapShot.pulsarFreq * rate;
  waveformFreq = std::max(waveformFreq, 20.0);
  cycleSamples = getSampleRate() / waveformFreq;
  // cycleSamples = snapShot.pulsarFreq * rate < 20 ? static_cast<double>(snapShot.pulsarPeriodTime * getSampleRate()) : cycleSamples; // 限制最长为500ms一个grain
  // cycleSamples = snapShot.pulsarFreq * rate < 20 std::min(cycleSamples, static_cast<double>(snapShot.pulsarDutyCycleSamples * 2)); // 限制最长为500ms一个grain
  ...
  double durCycles = std::max(0.05, static_cast<double>(snapShot.dutyCycleCluster) * dilation);
