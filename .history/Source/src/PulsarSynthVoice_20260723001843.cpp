...
void PulsarSynthVoice::spawnGrain() {
  ...
  cycleSamples = std::min(cycleSamples, snapShot.trainDutyCycleSamples);         // 限制最长为500ms一个grain
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  waveformSample = activeCount <= 0 ? 0.0f : static_cast<float>(1.0f * grainSum / std::pow(static_cast<double>(activeCount), 0.5));
