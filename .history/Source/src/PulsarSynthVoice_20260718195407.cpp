...
void PulsarSynthVoice::spawnGrain() {
  ...
  float perlinMod = getPerlinModulation();
  float caPitchOffset = (0.9f + 0.3f * (static_cast<float>(active) / 8.0f)) * perlinMod;
  caPitchOffset = 1.0f;
  float startPhaseOffset = static_cast<float>(active) / 8.0f * perlinMod;
  float ampRandom = (2.0f * static_cast<float>(active) / 8.0f / 10.0f - 0.1) * perlinMod;
  ampRandom = 1.0f;
  ...
  int samples = currentStateDurationSampleNum * 2;
...
float PulsarSynthVoice::calcActualPulse() {
  float phaseInc = 1.0f / getSampleRate();
  ...
  for (auto &g : waveformGrains) {
    ...
    float raw = std::sin(juce::MathConstants<float>::twoPi * g.phase);
    ...
    float completeWindow = gaussianEnvelope(g.windowPhase, 0.5);
    ...
    grainSum += raw; // am modulation
    activeCount++;
    if (g.phase == 0.5f) {
      DBG("phase=" << g.phase << " raw=" << raw << " remain=" << g.remainSamples);
    }

    // g.phase += g.phaseInc * fmLfo; // fm modulation
    g.phase += phaseInc; // fm modulation
    // if (g.phase >= 1.0f)
    //   g.phase -= 1;
    g.phase = fmod(g.phase, 1.0f);
    ...
    // g.phase = (g.totalSamples - g.remainSamples) / static_cast<float>(g.totalSamples);
    int played = g.totalSamples - g.remainSamples;
    float expected = std::fmod(played * phaseInc, 1.0f);
    if (g.phase != expected) {
      DBG("played=" << played << " expected=" << expected << " actual=" << g.phase);
    }
