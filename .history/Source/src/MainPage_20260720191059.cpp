...
void MainPage::initUITriggerEvent() {
  ...
  pgWaveformLoadFileButton.onClick = [this] {
    ...
    chooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles, [this, chooser](const juce::FileChooser &fc) {
      ...
      // wavetable scanning：把文件均分为多个连续段，每段重采样为一帧(2048点)，pulsar在train进程中依次扫描各帧
      const float *src = srcBuffer.getReadPointer(0);
      int srcLen = srcBuffer.getNumSamples();
      int numFrames = CommonVoiceSate::WAVETABLE_FRAMES;
      if (srcLen / numFrames < 2)
        numFrames = 1; // 文件太短，退化为单帧
      
      std::vector<std::array<float, targetSize>> frames(static_cast<size_t>(numFrames));
      for (int f = 0; f < numFrames; ++f) {
        int frameStart = static_cast<int>(static_cast<int64>(srcLen) * f / numFrames);
        int frameEnd = static_cast<int>(static_cast<int64>(srcLen) * (f + 1) / numFrames); // exclusive
        ...
        for (int i = 0; i < targetSize; ++i) {
          ...
          frames[f][i] = src[idx0] + frac * (src[idx1] - src[idx0]);
        }
      }

      // normalize：全局统一峰值归一化到[-1, 1]，保留帧间相对响度，避免load进来的音频过小(或过大被削波)
      float peak = 0.0f;
