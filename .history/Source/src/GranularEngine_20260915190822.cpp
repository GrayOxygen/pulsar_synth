...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    double raw = getWaveformEnvelopeValueAtPhase(static_cast<float>(g.windowPhasw), static_cast<float>(wtScanPos), perCycleWindow);
    ...
    if (g.windowPhase > 1.0)
      g.windowPhase = 1.0;
    // adsrWindow * completeWindow
    const double contribution = raw * g.amp * pulsaretEnvelope;
