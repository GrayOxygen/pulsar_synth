...
void PulsarSynthVoice::spawnGrain() {
  ...
  double triggerFreq = baseFreq * calcFmEnvelopeInterpolation(envPhase, commonVoiceSate->fmEnvelopeDepthParam->load()) * caPitchOffset;
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double fmLfo = calcFmLfoInterpolation(g.phase, commonVoiceSate->fmLfoDepthParam->load());
    double amLfo = calcAmLfoInterpolation(g.phase, commonVoiceSate->amLfoDepthParam->load());

    // 跨pulsar duty cycle window
    double completeWindow = gaussianEnvelope(g.windowPhase, 0.5);
