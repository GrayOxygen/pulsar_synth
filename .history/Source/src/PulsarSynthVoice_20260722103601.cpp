...
void PulsarSynthVoice::spawnGrain() {
  ...
  double cycles = std::max(1, static_cast<int>(std::round(snapShot.dutyCycleCluster * overlapMult))); // grain内包含的完整周期数(取整)
  ...
  if (slot >= 0) {
    waveformGrains[slot] = {0.5 * active / 8.0f, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, wtPos, wtPosInc};
