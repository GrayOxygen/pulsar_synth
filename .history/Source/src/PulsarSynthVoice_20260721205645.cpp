...
void PulsarSynthVoice::spawnGrain() {
  ...
  // NuPG式调制：全局LFO自由运行(processSample里逐sample推进，不随trigger重置)，
  // 在trigger瞬间Latch一次，冻结进本grain的rate和amp；grain内不再逐sample调制。
  // LFO速率跟随最新grain时长(一个grain生命周期对应一个LFO周期)
  ...
  phaseInc *= fmLatch;                    // 整个grain用冻结的播放速率(对应NuPG的 rate * (1 + Latch(...)))
