...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    float phase = std::fmod(static_cast<float>(fmLatch) / juce::MathConstants<float>::twoPi + driftPitch, 1.0f);
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  double tempLfoPhase = fmLatch;
  float normalizedPhase = std::fmod(static_cast<float>(tempLfoPhase) / juce::MathConstants<float>::twoPi + driftPitch, 1.0f);
  float triangle = 1.0f - 4.0f * std::abs(normalizedPhase - 0.5f);
  scanFreq *= (1.0f + triangle * fund);
  ...
  // wtPosInc *= (1.0f + std::sin(fmLatch * juce::MathConstants<float>::twoPi + driftPitch));
  ...
  // lfoPhaseInc *= (1.0f + std::sin(fmLatch * juce::MathConstants<float>::twoPi));
