...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  // 长度和p解耦
  int playbackSamples = static_cast<int>(std::round(getSampleRate() / 1000.0f * durCycles));

  // 一个 pulsaret 在 d 时间内完整扫完一次 waveform
  // const double phaseInc = 1.0 / playbackSamples;
  double scanFreq = fmSlice / (playbackSamples / getSampleRate());
  double phaseInc = scanFreq / getSampleRate();
