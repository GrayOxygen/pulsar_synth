...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    float raw = commonVoiceSate->usePgWaveformEnvelope.load() ? getWaveformEnvelopeValueAtPhase(globalGrainPhase) : 0.0f;
    raw = juce::jlimit(-1.0f, 1.0f, raw);

    float fmLfo = calcFmLfoInterpolation(globalGrainPhase, commonVoiceSate->fmLfoDepthParam->load());
    float amLfo = calcAmLfoInterpolation(globalGrainPhase, commonVoiceSate->amLfoDepthParam->load());
    ...
    grainSum += raw * completeWindow * windowInCurrentStage * getPulseFadeGain() * g.amp * amLfo; // am modulation
    activeCount++;
