...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  const float pan = getCurrentPan(envTimelinePhase(envIndex, kPanEnvTimelineRate, commonVoiceState->kEnvIndexWrapParam.load()), commonVoiceSate->panDepthParam->load());
  if (maskPass) {
