...
void PulsarSynthVoice::spawnGrain() {
  ...
  float triggerFreq = calcFmEnvelopeInterpolation(envPhase, commonVoiceSate->fmEnvelopeDepthParam->load()) * caPitchOffset;
  float phaseInc = (triggerFreq > 0.0f) ? triggerFreq / static_cast<float>(getSampleRate()) : 1.0f / static_cast<float>(getSampleRate());

  int samples = (currentStateDurationSampleNum) * static_cast<int>(getCurrentDutyCycleCluster(envPhase, commonVoiceSate->dutyCycleClusterDepthParam->load()));
  ...
  delaySamples = 0;
  waveformGrains[waveformGrainWriteIdx] = {0, phaseInc, true, samples, samples, delaySamples, 0, amp};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    activeCount++;
    g.phase += g.phaseInc * fmLfo * getPerlinModulation(); // fm modulation
    if (g.phase >= 1.0f)
      g.phase -= 1;
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) { return 0.01f + getFmLfoValueAtPhase(phase) * depth * 0.99; }

float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) { return 0.01f + getAmLfoValueAtPhase(phase) * depth * 0.99; }
