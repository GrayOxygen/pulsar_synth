...
void EnvelopeCanvas::setEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data, bool notify) {
  envelopeData = data;
  repaint();
  // wavetable多帧加载时只展示第一帧，不广播；否则canvas listener会把synth的wavetable覆盖回单帧
  if (notify)
    sendChangeMessage();
}
