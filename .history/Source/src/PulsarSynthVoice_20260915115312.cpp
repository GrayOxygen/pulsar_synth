...
constexpr int kEnvIndexWrap = EnvelopeCanvas::ENVELOPE_SIZE;
inline float envTimelinePhase(int envIndex, double rate) { return static_cast<float>(std::fmod(static_cast<double>(envIndex) * rate / EnvelopeCanvas::ENVELOPE_SIZE, 1.0)); }
} // namespace
...
void PulsarSynthVoice::changeStage() {
  ...
  case why::PulsarStateEnum::IntraSilence: // The silence stage of pulsar has now been completed
    ...
    // envIndex += static_cast<int>(std::sin(fmLatch * juce::MathConstants<float>::twoPi) * EnvelopeCanvas::ENVELOPE_SIZE);
  ...
  case why::PulsarStateEnum::InterTrainSilence: // train silence finished，开启新train
    ...
    maskGatedSpawn();
    envIndex++;
    // envIndex += std::sin(fmLatch * juce::MathConstants<float>::twoPi) * EnvelopeCanvas::ENVELOPE_SIZE;
    if (envIndex >= kEnvIndexWrap)
      envIndex -= kEnvIndexWrap;
    if (envIndex < 0) {
      envIndex += kEnvIndexWrap;
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  if (amp <= 1.0e-4f)
    return;
  // 大于1，长度越短，频率越高，小于1，长度越长，频率越高，overlap部分越多
  ...
  // rateMod只改播放速率(音高)，不改grain时长——
  // playbackSamples仍由未调制的cycleSamples决定，phaseInc乘上trigger时Latch的调制值。
  // 速率只含显式unison失谐(extraDetuneCents，非unison grain为0)：
  // 同源grain速率完全一致=重叠相干清晰；unison拷贝±对称失谐=稳定的缓慢拍频厚度
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  ...
  // float ratio = std::pow(2.0f, semitone * depth / 96000.0f);
  // return ratio;
