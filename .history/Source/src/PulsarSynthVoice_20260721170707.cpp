...
void PulsarSynthVoice::spawnGrain() {
  ...
  waveformGrains[waveformGrainWriteIdx] = {lastPhase, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, lfoPhase, lfoPhaseInc, wtPos};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double fmLfo = calcFmLfoInterpolation(static_cast<float>(g.lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    double amLfo = calcAmLfoInterpolation(static_cast<float>(g.lfoPhase), commonVoiceSate->amLfoDepthParam->load());
  ...
  waveformSample = activeCount > 0 ? grainSum；
