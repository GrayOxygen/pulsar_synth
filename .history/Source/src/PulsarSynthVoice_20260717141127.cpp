...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    if (g.remainSamples <= 1)
      completeWindow = 0.0f;
    float pulsarWindowPhase = fmod(static_cast<float>(currentStateDurationSampleNum - g.remainSamples), static_cast<float>(currentStateDurationSampleNum / g.remainSamples - 1));
    float pulsarWindow = gaussianEnvelope(pulsarWindowPhase, 0.5f);

    grainSum += raw * completeWindow * pulsarWindow * g.amp * amLfo; // am modulation
    activeCount++;
