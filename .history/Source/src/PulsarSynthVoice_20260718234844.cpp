...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * completeWindow * windowInCurrentStage * g.amp * ampLfo; // am modulation
    activeCount++;
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  return 0.5f + (2.0f * getFmLfoValueAtPhase(phase) * depth * -1.0f) / 2.0f; // [0, 0.5]
}

float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) {
  return 1.0f + (2.0f * getAmLfoValueAtPhase(phase) * depth * -1.0f); // [0, 1]
}
