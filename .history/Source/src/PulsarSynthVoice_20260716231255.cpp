...
void PulsarSynthVoice::mappingParams(const juce::AudioProcessorValueTreeState &apvts) {
  ...
  commonVoiceSate->fmEnvelopeDepthParam = apvts.getRawParameterValue(why::ParameterID::formantFreqLfoDepth);
...
void PulsarSynthVoice::spawnGrain() {
  ...
  int samples = currentStateDurationSampleNum * getCurrentDutyCycleCluster(envPhase, commonVoiceSate->dutyCycleClusterDepthParam->load());
...
float PulsarSynthVoice::getPulseFadeGain() const {
  ...
  return gaussianEnvelope(static_cast<float>(pos) / static_cast<float>(N - 1));
}
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    const float window = gaussianEnvelope(g.windowPhase);
    grainSum += raw * window * g.amp * (1.0f + (amLfo - 0.5f)); // am modulation
    activeCount++;
    g.phase += g.phaseInc + (fmLfo - 0.5f) / getSampleRate(); // fm modulation
    if (g.phase >= 1.0f)
      g.phase -= 1;
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) { return (getFmLfoValueAtPhase(phase) - 0.5f) * depth; }
float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) { return (1.0f - depth) + getAmLfoValueAtPhase(phase) * depth; }
