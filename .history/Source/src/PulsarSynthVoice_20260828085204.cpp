...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  r = r + static_cast<int>(perlinNoise.noise(static_cast<float>(perlinTime * 0.31 + 17.0)) * 100);
  ...
  if (maskPass) {
    ...
    if (r % 5 == 1) {
      spawnGrain(snapShot.fundamentalFreq, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.75f);
      spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.5f);
    }
    if (r % 5 == 2) {
      spawnGrain(snapShot.fundamentalFreq, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 3.0f, 0.75f);
      spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.5f);
    }
    if (r % 5 == 3) {
      spawnGrain(snapShot.fundamentalFreq, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.5f);
      spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.25f);
      spawnGrain(snapShot.fundamentalFreq * 6.0f, 0.2f);
      spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.1f);
    }
    if (r % 5 == 4) {
      spawnGrain(snapShot.fundamentalFreq, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 2.0f, 0.75f);
      spawnGrain(snapShot.fundamentalFreq * 3.0f, 0.7f);
      spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.7f);
      spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.65f);
      spawnGrain(snapShot.fundamentalFreq * 6.0f, 0.55f);
      spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.5f);
      spawnGrain(snapShot.fundamentalFreq * 8.0f, 0.25f);
      spawnGrain(snapShot.fundamentalFreq * 9.0f, 0.1f);
      spawnGrain(snapShot.fundamentalFreq * 10.0f, 0.1f);
      spawnGrain(snapShot.fundamentalFreq * 11.0f, 0.1f);
    }

    double fmEnvelope = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    spawnGrain(snapShot.fundamentalFreq * 11.0f * fmEnvelope, 0.05f);
  }
}
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor) {
  ...
  int delaySamples = 0;
  double startPhase = 0;
  delaySamples = r < 0.3 ? static_cast<int>(active + driftPhase * snapShot.pulsarDutyCycleSamples) : 0;
  startPhase = r > 0.3 ? static_cast<double>(active / 8.0f + driftPhase) : 0.0;
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double fmEnvelope = calcFmLfoInterpolation(static_cast<float>(g.windowPhase), commonVoiceSate->fmLfoDepthParam->load());
    ...
    g.phase += g.phaseInc * std::sin(g.fm * fmEnvelope);
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
