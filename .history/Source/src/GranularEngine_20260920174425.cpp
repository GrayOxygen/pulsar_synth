...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    double pulsaretFMEnvelope = calcFmLfoInterpolation(static_cast<float>(g.windowPhase), commonVoiceSate->fmLfoDepthParam->load());
    ...
    g.phase += g.phaseInc * (1.0f + 0.1f * pulsaretFMEnvelope);
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
  ...
  wtScanPos += wtScanInc * (1 + commonVoiceSate->unisonWidthParam->load());
  // wtScanPos += wtScanInc;
