...
void GranularEngine::spawn(const SpawnParams &p) {
  if (p.fund) {
    ...
    wtScanInc = (wtScanInc < 0.0 ? -1.0 : 1.0) / (EnvelopeCanvas::ENVELOPE_SIZE * p.playbackSamples);
...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    const bool perCycleWindow = true;
    ...
    float adsrA = commonVoiceState->grainAdsrAttack.load();
    float adsrD = commonVoiceState->grainAdsrDecay.load();
    float adsrS = commonVoiceState->grainAdsrSustain.load();
    float adsrR = commonVoiceState->grainAdsrRelease.load();
    ...
    double pulsaretAMEnvelope = calcAmLfoInterpolation(static_cast<float>(g.windowPhase), commonVoiceState->amLfoDepthParam->load());
    double pulsaretFMEnvelope = calcFmLfoInterpolation(static_cast<float>(g.windowPhase), commonVoiceState->fmLfoDepthParam->load());
    ...
    if (g.windowPhase > 1.0)
      g.windowPhase = 1.0;
    const double contribution = raw * g.amp * pulsaretAMEnvelope * adsrWindow * completeWindow;
    // per-grain equal-power pan(√2补偿使居中=unity gain，与旧mono路径响度一致)：
    // pan由voice在spawn时从pan包络latch，pan=0走中心不变
    ...
    g.phase += g.phaseInc * (1 + r * pulsaretFMEnvelope);
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
  ...
  wtScanPos += wtScanInc * commonVoiceState->unisonWidthParam->load();
  // wtScanPos += wtScanInc;
  // ping-pong反弹代替wrap：wrap(末帧->首帧)的音色突变每扫完一圈crack一次，反弹让morph轨迹连续来回、无接缝
  if (wtScanPos >= 1.0) {
...
float GranularEngine::calcFmLfoInterpolation(float phase, float depth) {
  ...
  return depth * getFmLfoValueAtPhase(phase);
}

float GranularEngine::getFmLfoValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceState->fmLfoData, phase, commonVoiceState->fmLfoScale.load()); }

float GranularEngine::getWaveformEnvelopeValueAtPhase(float phase, float wtPos, bool perCycleWindow) const {
  const float scale = commonVoiceState->pgWaveformEnvelopeScale.load();
  const int numFrames = commonVoiceState->pgWavetableNumFrames.load();

  // wavetable scanning：wtPos(0~1)映射到帧序列，相邻两帧线性morph；单帧时退化为原来的单波形查表
  float v, dc;
  if (numFrames <= 1) {
    v = getEnvelopeValueAtPhase(commonVoiceState->pgWavetable[0], phase, scale);
    dc = commonVoiceState->pgWavetableDcOffset[0].load();
  } else {
    ...
    float v0 = getEnvelopeValueAtPhase(commonVoiceState->pgWavetable[f0], phase, scale);
    float v1 = getEnvelopeValueAtPhase(commonVoiceState->pgWavetable[f1], phase, scale);
    v = v0 * (1.0f - t) + v1 * t;
    dc = commonVoiceState->pgWavetableDcOffset[f0].load() * (1.0f - t) + commonVoiceState->pgWavetableDcOffset[f1].load() * t;
    ...
    // float v0 = getEnvelopeValueAtPhase(commonVoiceState->pgWavetable[currentFrame], phase, scale);
    // float v1 = getEnvelopeValueAtPhase(commonVoiceState->pgWavetable[nextFrame], phase, scale);
    // float dc0 = commonVoiceState->pgWavetableDcOffset[currentFrame].load();
    // float dc1 = commonVoiceState->pgWavetableDcOffset[nextFrame].load();
    // v = v0 * (1.0f - t) + v1 * t;
    // dc = dc0 * (1.0f - t) + dc1 * t;
    // if (wtPos < lastWtPos) {
    //   currentFrame = nextFrame;
...
float GranularEngine::calcAmLfoInterpolation(float phase, float depth) const {
  return getAmLfoValueAtPhase(phase) * depth; // [0, 1]
}

float GranularEngine::getAmLfoValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceState->amLfoData, phase, commonVoiceState->amLfoScale.load()); }
