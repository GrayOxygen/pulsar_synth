...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    if (g.remainSamples <= 1)
      completeWindow = 0.0f;
    grainSum += raw * completeWindow *
                gaussianEnvelope(
                    static_cast<float>(gaussianEnvelope(static_cast<float>(pos) / static_cast<float>(N - 1), 0.5) - g.remainSamples) / static_cast<float>(currentStateDurationSampleNum - 1), 0.5) *
                g.amp * amLfo; // am modulation
    activeCount++;
