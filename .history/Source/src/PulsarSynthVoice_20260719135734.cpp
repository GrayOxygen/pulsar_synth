...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * amLfo * g.scale; // am modulation
    // grainSum += raw * completeWindow * getPulseFadeGain() * windowInCurrentStage ; // am modulation
    grainSum = juce::dsp::FastMathApproximations::tanh(grainSum) * completeWindow * getPulseFadeGain() * windowInCurrentStage  *g.amp;
    activeCount++;
