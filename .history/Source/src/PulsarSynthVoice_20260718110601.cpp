...
float PulsarSynthVoice::calSampleByState(bool passMaskFlag, bool existMask) {
  ...
  case why::PulsarStateEnum::IntraSilence: {
    ...
    // if (hasPassedSampleNumInsideTrain >= currentStateDurationSampleNum - 5) {
    //   DBG("pos=" << hasPassedSampleNumInsideTrain << " gain=" << gain);
    // }
...
void PulsarSynthVoice::spawnGrain() {
  ...
  for (int j = 0; j < 8; ++j)
    active += (caState >> j) & 1;
  float caPitchOffset = (0.9f + 0.3f * (static_cast<float>(active) / 8.0f)) * getPerlinModulation();
  float startPhaseOffset = static_cast<float>(active) / 8.0f * getPerlinModulation();
  float ampRandom = (2.0f * static_cast<float>(active) / 8.0f / 10.0f - 0.1) * getPerlinModulation();

  float envPhase = 1.0f * envIndex / 2048;
  // 获取实时envelope; 每个sample，对应一个envelope点，确定fm，am，dc ratio，dc cluster，然后用确定的freq扫描waveform
  float triggerFreq = snapShot.pulsarFreq * calcFmEnvelopeInterpolation(envPhase, commonVoiceSate->fmEnvelopeDepthParam->load()) * caPitchOffset;
  float phaseInc = (triggerFreq > 0.0f) ? triggerFreq / static_cast<float>(getSampleRate()) : 1.0f / static_cast<float>(getSampleRate());

  // int samples = static_cast<int>(((currentStateDurationSampleNum)) * static_cast<float>(getCurrentDutyCycleCluster(envPhase, commonVoiceSate->dutyCycleClusterDepthParam->load())));
  int samples = currentStateDurationSampleNum;
  // float amp = calcAmEnvelopeInterpolation(envPhase, commonVoiceSate->amEnvDepthParam->load()) * (1 + ampRandom);
  float amp = calcAmEnvelopeInterpolation(envPhase, commonVoiceSate->amEnvDepthParam->load());
  int delaySamples = static_cast<int>(currentStateDurationSampleNum * (active / 8.0f)) * ampRandom;
  waveformGrains[waveformGrainWriteIdx] = {0, phaseInc, true, samples, samples, 0, 0, amp};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    float raw = getWaveformEnvelopeValueAtPhase(g.phase);
    raw = juce::jlimit(-1.0f, 1.0f, raw);

    float fmLfo = calcFmLfoInterpolation(g.phase, commonVoiceSate->fmLfoDepthParam->load());
    float amLfo = calcAmLfoInterpolation(g.phase, commonVoiceSate->amLfoDepthParam->load());

    // 跨pulsar duty cycle window
    float completeWindow = gaussianEnvelope(g.windowPhase, 3.5);
    if (g.remainSamples <= 1)
      completeWindow = 0.0f;

    float windowInCurrentStage = gaussianEnvelope(static_cast<float>(hasPassedSampleNumInsideTrain) / static_cast<float>(g.samplesInCurrentStage - 1), 0.5);
    if (hasPassedSampleNumInsideTrain >= g.samplesInCurrentStage) {
      windowInCurrentStage = 0;
    }
    //  completeWindow * windowInCurrentStage * g.amp * amLfo
    grainSum += raw * completeWindow * g.amp * amLfo; // am modulation
    activeCount++;
    g.phase += g.phaseInc * fmLfo; // fm modulation
    if (g.phase >= 1.0f)
      g.phase -= 1;

    // globalGrainPhase += g.phaseInc; // fm modulation
    // if (globalGrainPhase >= 1.0f)
    //   globalGrainPhase -= 1;
    g.windowPhase += 1.0f / static_cast<float>(g.totalSamples);
  ...
  waveformSample = activeCount > 0 ? grainSum / std::pow(activeCount, 0.7f) : 0.0f;
  // waveformSample = activeCount > 0 ? grainSum / static_cast<float>(sqrt(activeCount)) : 0.0f;
...
float PulsarSynthVoice::getCurrentDutyCycleCluster(float phase, float depth) {
  ...
  if (commonVoiceSate->useDutyCycleClusterEnvelope.load()) {
    ...
    return static_cast<float>(depth * envelopeValue);
  }
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  return 0.5f + getFmLfoValueAtPhase(phase) * depth * 1.5f; // [0.5, 2]
}

float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) {
  return 0.1f + getAmLfoValueAtPhase(phase) * depth * 0.9; // [0.1, 1]
}
...
float PulsarSynthVoice::getAmpEnvelopeValueAtPhase(float phase) const { return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->ampEnvelopeData, phase, commonVoiceSate->ampEnvelopeScale.load()); }

float PulsarSynthVoice::getFmEnvelopeValueAtPhase(float phase) const { return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->fmEnvelopeData, phase, commonVoiceSate->fmEnvelopeScale.load()); }

float PulsarSynthVoice::getDutyCycleRatioEnvelopeValueAtPhase(float phase) const {
  return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->dutyCycleRatioEnvelopeData, phase, commonVoiceSate->dutyCycleRatioEnvelopeScale.load());
}

float PulsarSynthVoice::getDutyCycleClusterEnvelopeValueAtPhase(float phase) const {
  return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->dutyCycleClusterEnvelopeData, phase, commonVoiceSate->dutyCycleClusterEnvelopeScale.load());
}

float PulsarSynthVoice::getWaveformEnvelopeValueAtPhase(float phase) const {
  return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->pgWaveformEnvelopeData, phase, commonVoiceSate->pgWaveformEnvelopeScale.load());
}
float PulsarSynthVoice::getFmLfoValueAtPhase(float phase) const { return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->fmLfoData, phase, commonVoiceSate->fmLfoScale.load()); }
float PulsarSynthVoice::getAmLfoValueAtPhase(float phase) const { return EnvelopeCanvas::getEnvelopeValueAtPhase(commonVoiceSate->amLfoData, phase, commonVoiceSate->amLfoScale.load()); }
