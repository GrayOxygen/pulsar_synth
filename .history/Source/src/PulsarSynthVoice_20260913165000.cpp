...
// ============================ envelope时间线去锁定 ============================
// 各调制包络的时间线速率比(基准=envIndex/2048，每个pulsar period前进一步)：
...
// envIndex放大wrap上限(2048的整数倍，保证rate=1.0的amp包络在wrap处无缝)；
// 相位在double域取模后再降为float，避免大index直接除法的精度损失
constexpr int kEnvIndexWrap = 2048 * 4096;
inline float envTimelinePhase(int envIndex, double rate) { return static_cast<float>(std::fmod(static_cast<double>(envIndex) * rate / 2048.0, 1.0)); }
} // namespace
...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    if (harmonics > 1.0e-3f) {
      ...
      // spawnGrain(snapShot.fundamentalFreq * 4.0, 0.5*harmonics / 4.0 * (0.35f + 0.65f * h4) * bright * bright, 1.0f, false, 1.0f);
    }
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  // amp *= 1.0f + driftAmp * 0.1f;
  // per-grain随机音高微漂已移除：cluster>1时N份重叠拷贝各latch一个不同的随机失谐
  // (哪怕只有±3 cents)=N路声部互相拍频/相位游走——就是重叠模式的"合唱感/不清晰"。
  // 重叠拷贝必须严格同速率才能相干叠加成干净的增强/梳状；想要失谐厚度请用
  // 显式的unison detune(±对称、稳定拍频)，而不是随机per-grain漂移
  juce::ignoreUnused(fmLatch, driftPitch);
  ...
  // 注意：之前的(1+driftPitch*2)因子里driftPitch(perlin)值域±2，速率因子在[-3,+5]间漂，
  // 慢速perlin漂到-0.5以下的整段时间里所有grain速率≤0=波形冻在phase 0(hann窗=0)——
  // 周期性完全静音几秒的元凶。改用cents级detune(±3c有机漂移+unison固定失谐)：
  // 比值恒>0且≈1，音高只有丝般微漂，永不哑掉
  double detuneRatio = std::pow(2.0, (static_cast<double>(detuneCents) + extraDetuneCents) / 1200.0);
  ...
  const double periodSamples = static_cast<double>(snapShot.pulsarPeriodTime) * getSampleRate();
