...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    g.phase += g.phaseInc * (1 + r * 0.1f + 0.9f * pulsaretFMEnvelope / activeCount);
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
  ...
  // wtScanPos += wtScanInc * 32.0f * commonVoiceSate->unisonWidthParam->load() * (0.5f + r / 2.0f);
  wtScanPos += wtScanInc;
...
float GranularEngine::calcFmLfoInterpolation(float phase, float depth) {
  ...
  return 1 + depth * (getFmLfoValueAtPhase(phase) - 1);
}
