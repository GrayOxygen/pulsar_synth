...
#include "../include/commonVoiceState.h"
#include "LfoPage.h"
#include <algorithm>
#include <cmath>
...
constexpr double kEnvLoopPeriods = 9600.0;
inline float envTimelinePhase(int envIndex, double rate, int kEnvIndexWrap) { return static_cast<float>(std::fmod(static_cast<double>(envIndex) * rate * kEnvIndexWrap / kEnvLoopPeriods, 1.0)); }
} // namespace
...
void PulsarSynthVoice::saveSnapShot() {
  ...
  snapShot.trainDutyCycleLenParam = commonVoiceState->trainDutyCycleLenParam->load();
  snapShot.trainSilenceParam = commonVoiceState->trainSilenceParam->load();
  snapShot.trainLenParam = commonVoiceState->trainLenParam->load();
  ...
  snapShot.dutyCycleCluster =
      getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate, commonVoiceState->envelopeScanSpeedParam->load()), commonVoiceState->dutyCycleClusterDepthParam->load());
...
void PulsarSynthVoice::processBlockSamples(juce::AudioSampleBuffer &outputBuffer, int startSample, int numSamples) {
  commonVoiceState->pulseBuffer.clear();
  if (commonVoiceState->pulseBuffer.getNumSamples() < numSamples || commonVoiceState->pulseBuffer.getNumChannels() < outputBuffer.getNumChannels()) {
    commonVoiceState->pulseBuffer.setSize(outputBuffer.getNumChannels(), numSamples, false, true, true); // 自动释放并重新分配
  }
  ...
  for (int sampleIndex = startSample; sampleIndex < (startSample + numSamples); sampleIndex++) {
    ...
    if (commonVoiceState->pulseBuffer.getNumChannels() > 1) {
      commonVoiceState->pulseBuffer.setSample(0, i, l);
      commonVoiceState->pulseBuffer.setSample(1, i, r);
    } else {
      commonVoiceState->pulseBuffer.setSample(0, i, 0.5f * (l + r));
    }
  }
  ...
  // pan已改为per-grain：spawn时从pan包络latch(见maskGatedSpawn)，由GranularEngine
  // 按equal-power渲染到左右声道；这里不再做block级pan，避免双重panning
  for (int sampleIndex = startSample; sampleIndex < (startSample + numSamples); sampleIndex++) {
    int i = sampleIndex - startSample;
    for (int chan = 0; chan < numChannels; chan++) {
      const auto *pulseReadPtr = commonVoiceState->pulseBuffer.getReadPointer(chan);
      ...
      const float s = juce::dsp::FastMathApproximations::tanh(juce::jlimit(-4.0f, 4.0f, pulseReadPtr[i])) * outputGain;
...
void PulsarSynthVoice::initSynthVoice(double sampleRate, juce::AudioPlayHead *audioPlayHead) {
  ...
  commonVoiceState->generateStochasticMask();
}

void PulsarSynthVoice::mappingParams(const juce::AudioProcessorValueTreeState &apvts) {
  // output
  commonVoiceState->outputGainParam = apvts.getRawParameterValue(why::ParameterID::outputGain);

  // train
  commonVoiceState->trainLenParam = apvts.getRawParameterValue(why::ParameterID::trainLen);
  commonVoiceState->trainDutyCycleLenParam = apvts.getRawParameterValue(why::ParameterID::trainDutyCycleLen);
  commonVoiceState->trainSilenceParam = apvts.getRawParameterValue(why::ParameterID::trainSilenceLen);

  // pulsar basic info
  commonVoiceState->pulsarWaveformParam = apvts.getRawParameterValue(why::ParameterID::pulsarWaveform);

  //  envelope depth
  commonVoiceState->amEnvDepthParam = apvts.getRawParameterValue(why::ParameterID::amEnvelopeDepth);
  commonVoiceState->fmEnvelopeDepthParam = apvts.getRawParameterValue(why::ParameterID::fmEnvelopeDepth);
  commonVoiceState->panDepthParam = apvts.getRawParameterValue(why::ParameterID::panDepth);
  commonVoiceState->dutyCycleClusterDepthParam = apvts.getRawParameterValue(why::ParameterID::dutyCycleClusterDepth);
  commonVoiceState->fmLfoDepthParam = apvts.getRawParameterValue(why::ParameterID::fmLfoDepth);
  commonVoiceState->amLfoDepthParam = apvts.getRawParameterValue(why::ParameterID::amLfoDepth);

  // pulsar extension
  commonVoiceState->harmonicsParam = apvts.getRawParameterValue(why::ParameterID::harmonics);
  commonVoiceState->unisonDetuneParam = apvts.getRawParameterValue(why::ParameterID::unisonDetune);
  commonVoiceState->unisonWidthParam = apvts.getRawParameterValue(why::ParameterID::waveScanSpeed);

  // masking
  commonVoiceState->maskOptionParam = apvts.getRawParameterValue(why::ParameterID::maskOption);
  commonVoiceState->maskOption = static_cast<why::MaskOptionEnum>(static_cast<int>(commonVoiceState->maskOptionParam->load()));
  ...
  if (!apvts.state.getProperty(why::PropertyID::burstMask).isVoid()) {
    ...
    commonVoiceState->burstMask = apvts.state.getProperty(why::PropertyID::burstMask).toString().toStdString();
  }

  // stochastic mask
  if (!apvts.state.getProperty(why::PropertyID::stochasticMask).isVoid()) {
    commonVoiceState->stochasticMaskStr = apvts.state.getProperty(why::PropertyID::stochasticMask).toString().toStdString();
  }

  // euclid mask
  commonVoiceState->euclidStepsParam = apvts.getRawParameterValue(why::ParameterID::euclidSteps);
  commonVoiceState->euclidHitsParam = apvts.getRawParameterValue(why::ParameterID::euclidHits);

  if (commonVoiceState->euclidStepsParam->load() > 0 && commonVoiceState->euclidHitsParam->load() > 0) {
    // generate euclid rhythm pattern
    commonVoiceState->euclids = why::generateEuclidRhythm(commonVoiceState->euclidStepsParam->load(), commonVoiceState->euclidHitsParam->load());
  }

  commonVoiceState->envelopeScanSpeedParam = apvts.getRawParameterValue(why::ParameterID::envelopeScanSpeed);
}
...
void PulsarSynthVoice::mappingOneParam(const juce::AudioProcessorValueTreeState &apvts, juce::String parameterID, float newValue) {
  ...
  if (parameterID == why::ParameterID::outputGain) {
    commonVoiceState->outputGainParam->store(newValue);
  }

  // train
  if (parameterID == why::ParameterID::trainLen) {
    commonVoiceState->trainLenParam->store(newValue);
  }
  if (parameterID == why::ParameterID::trainDutyCycleLen) {
    commonVoiceState->trainDutyCycleLenParam->store(newValue);
  }
  if (parameterID == why::ParameterID::trainSilenceLen) {
    commonVoiceState->trainSilenceParam->store(newValue);
  }

  // pulsar basic info
  if (parameterID == why::ParameterID::pulsarWaveform) {
    commonVoiceState->pulsarWaveformParam->store(newValue);
  }

  // envelope  depth
  if (parameterID == why::ParameterID::amEnvelopeDepth) {
    commonVoiceState->amEnvDepthParam->store(newValue);
  }
  if (parameterID == why::ParameterID::fmEnvelopeDepth) {
    commonVoiceState->fmEnvelopeDepthParam->store(newValue);
  }
  if (parameterID == why::ParameterID::panDepth) {
    commonVoiceState->panDepthParam->store(newValue);
  }
  if (parameterID == why::ParameterID::dutyCycleClusterDepth) {
    commonVoiceState->dutyCycleClusterDepthParam->store(newValue);
  }
  if (parameterID == why::ParameterID::fmLfoDepth) {
    commonVoiceState->fmLfoDepthParam->store(newValue);
  }
  if (parameterID == why::ParameterID::amLfoDepth) {
    commonVoiceState->amLfoDepthParam->store(newValue);
  }

  // pulsar extension
  if (parameterID == why::ParameterID::harmonics && commonVoiceState->harmonicsParam != nullptr) {
    commonVoiceState->harmonicsParam->store(newValue);
  }
  if (parameterID == why::ParameterID::unisonDetune && commonVoiceState->unisonDetuneParam != nullptr) {
    commonVoiceState->unisonDetuneParam->store(newValue);
  }
  if (parameterID == why::ParameterID::waveScanSpeed && commonVoiceState->unisonWidthParam != nullptr) {
    commonVoiceState->unisonWidthParam->store(newValue);
  }
  // masking
  if (parameterID == why::ParameterID::maskOption) {
    commonVoiceState->maskOptionParam->store(newValue);
    commonVoiceState->maskOption = static_cast<why::MaskOptionEnum>(static_cast<int>(commonVoiceState->maskOptionParam->load()));
  }
  ...
  if (parameterID == why::ParameterID::euclidSteps) {
    commonVoiceState->euclidStepsParam->store(newValue);
  }
  if (parameterID == why::ParameterID::euclidHits) {
    commonVoiceState->euclidHitsParam->store(newValue);
  }
...
void PulsarSynthVoice::parameterChanged(juce::AudioProcessorValueTreeState &apvts, juce::String parameterID, float newValue, bool &isGeneratedStochasticMask) {
  ...
  if (parameterID == why::ParameterID::trainDutyCycleLen || parameterID == why::ParameterID::trainSilenceLen || parameterID == why::ParameterID::trainLen) {
    ...
    if (snapShot.trainSilenceParam != commonVoiceState->trainSilenceParam->load() || snapShot.trainLenParam != commonVoiceState->trainLenParam->load() ||
        snapShot.trainDutyCycleLenParam != commonVoiceState->trainDutyCycleLenParam->load()) {
      setEnterNextTrain(true);
      ...
      if (parameterID == why::ParameterID::trainDutyCycleLen) {
        ...
        if (isGeneratedStochasticMask) {
          if (apvts.state.getProperty(why::PropertyID::stochasticMask) != commonVoiceState->stochasticMaskStr.data()) {
            apvts.state.setProperty(why::PropertyID::stochasticMask, commonVoiceState->stochasticMaskStr.data(), nullptr);
          }
        }
      }
    }
...
float PulsarSynthVoice::getCurrentBpm() const {
  float bpm = commonVoiceState->bpmEnvelopeData[static_cast<size_t>(bpmEnvIndex)];
  return juce::jlimit(commonVoiceState->bpmEnvelopeYMin.load(), commonVoiceState->bpmEnvelopeYMax.load(), bpm);
}
...
void PulsarSynthVoice::mask(bool &maskPassFlag, bool &existMask) {
  if (commonVoiceState->maskOption == why::MaskOptionEnum::Off) {
    existMask = false;
    maskPassFlag = true;
    return;
  }
  switch (commonVoiceState->maskOption) {
  case why::MaskOptionEnum::BurstMask:
    applyMaskString(commonVoiceState->burstMask, existMask, maskPassFlag);
    break;
  case why::MaskOptionEnum::EuclidMask:
    applyMaskString(commonVoiceState->euclids, existMask, maskPassFlag);
    break;
  case why::MaskOptionEnum::StochasticMask:
    applyMaskString(commonVoiceState->stochasticMaskStr, existMask, maskPassFlag);
    break;
  default:
    break;
  }
...
void PulsarSynthVoice::changeStage() {
  ...
  case why::PulsarStateEnum::IntraSilence: // The silence stage of pulsar has now been completed
    ...
    // kEnvIndexWrap区间内自由发展，组合模式不会每个waveform shape就整体重演一次
    envIndex = envIndex + commonVoiceState->envelopeScanSpeedParam->load();
    if (envIndex >= kEnvLoopPeriods)
      envIndex -= kEnvLoopPeriods;
    if (envIndex < 0) {
      envIndex += kEnvLoopPeriods;
    }
    break;
...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  // pan包络：trigger时刻latch当前pan(-1~+1)，本次spawn的所有grain共用同一声像位置；
  // 包络沿自身无理数速率时间线推进，pulsaret随包络在声场中扫动，已播放grain的尾巴保持原位
  const float pan = getCurrentPan(envTimelinePhase(envIndex, kPanEnvTimelineRate, commonVoiceState->envelopeScanSpeedParam->load()), commonVoiceState->panDepthParam->load());
  if (maskPass) {
    spawnGrain(snapShot.fundamentalFreq, 1.0f, 1.0f, true, 1.0f, 0.0, pan);
    ...
    const float harmonics = commonVoiceState->harmonicsParam != nullptr ? commonVoiceState->harmonicsParam->load() : 0.0f;
    if (harmonics > 1.0e-3f) {
      ...
      spawnGrain(snapShot.fundamentalFreq * 3.0, 0.5 * harmonics / 3.0 * (0.35f + 0.65f * h2), 1.0f, false, 1.0f, 0.0, pan);
      spawnGrain(snapShot.fundamentalFreq * 5.0, 0.5 * harmonics / 5.0 * (0.35f + 0.65f * h3), 1.0f, false, 1.0f, 0.0, pan);
    }

    // unison失谐层：基频的±detune cents拷贝，pan同样跟随pan包络(与基频同位置)，
    // cents级失谐产生缓慢拍频=supersaw式厚度，不会像大失谐那样粗糙
    const float unisonDetune = commonVoiceState->unisonDetuneParam != nullptr ? commonVoiceState->unisonDetuneParam->load() : 0.0f;

    if (unisonDetune > 0.01f) {
      spawnGrain(snapShot.fundamentalFreq, 0.37f * h2, 1.0f, false, 1.0f, -unisonDetune, pan);
      // spawnGrain(snapShot.fundamentalFreq, 0.3f * h3, 1.0f, false, 1.0f, +unisonDetune, pan);
    }
  }
}
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  float amp = calcAmEnvelopeInterpolation(envTimelinePhase(envIndex, kAmEnvTimelineRate, commonVoiceState->envelopeScanSpeedParam->load()), commonVoiceState->amEnvDepthParam->load());
  double fmSlice = calcFmEnvelopeInterpolation(envTimelinePhase(envIndex, kFmEnvTimelineRate, commonVoiceState->envelopeScanSpeedParam->load()), commonVoiceState->fmEnvelopeDepthParam->load());
  ...
  double durCycles = static_cast<double>(snapShot.dutyCycleCluster) * (1 + active / 8.0f) * length;
  // 实际duration length
  double playbackSamples = static_cast<double>(static_cast<int>(durCycles)) * periodSamples;

  // 一个 pulsaret 在 d 时间内完整扫完durCycles次 waveform
  // detune调节 extraDetuneCents=50 偏移0.5个半音
  double detuneRatio = std::pow(2.0, extraDetuneCents / 1200.0);
  // 扫描频率
  double phaseInc = (1 + 0.1 * active + 0.1 * getPerlinModulation() + 0.7 * fmSlice) * detuneRatio * 1.0f / static_cast<double>(periodSamples);
  amp /= std::sqrt(std::max(1.0, durCycles));

  //  只提供trigger时刻算好的参数
  GranularEngine::SpawnParams params;
  params.startPhase = 0.0f; // 修改phase偏移，效果有点粗糙
...
float PulsarSynthVoice::calcFmEnvelopeInterpolation(float phase, float depth) { return depth * getFmEnvelopeValueAtPhase(phase); }
...
float PulsarSynthVoice::calcAmEnvelopeInterpolation(float phase, float depth) {
  ...
  if (depth <= 0.0f || !commonVoiceState->useAmpEnvelope.load()) {
    return 1.0f;
  }
...
//   return 1.0f + depth * (getFmLfoValueAtPhase(phase) - 1.0f);
// }
...
float PulsarSynthVoice::getAmpEnvelopeValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceState->ampEnvelopeData, phase, commonVoiceState->ampEnvelopeScale.load()); }

float PulsarSynthVoice::getFmEnvelopeValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceState->fmEnvelopeData, phase, commonVoiceState->fmEnvelopeScale.load()); }

float PulsarSynthVoice::getPanEnvelopeValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceState->panEnvelopeData, phase, commonVoiceState->panEnvelopeScale.load()); }

float PulsarSynthVoice::getDutyCycleClusterEnvelopeValueAtPhase(float phase) const {
  return getEnvelopeValueAtPhase(commonVoiceState->dutyCycleClusterEnvelopeData, phase, commonVoiceState->dutyCycleClusterEnvelopeScale.load());
}

// float PulsarSynthVoice::getFmLfoValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceState->fmLfoData, phase, commonVoiceState->fmLfoScale.load()); }

// db to gain
float PulsarSynthVoice::getOutputGain() { return commonVoiceState->outputGainParam == nullptr ? 1 : std::pow(10.0f, commonVoiceState->outputGainParam->load() / 20.0f); }
