...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  // 扫描频率
  double phaseInc = (1 + 0.2 * getPerlinModulation() + 0.1 * active / 8.0f) * (1 + fmSlice) * detuneRatio * 1.0f / static_cast<double>(periodSamples);
...
float PulsarSynthVoice::getCurrentDutyCycleCluster(float phase, float depth) {
  ...
  return 1 + depth * (envelopeValue - 1);
}
...
float PulsarSynthVoice::calcFmEnvelopeInterpolation(float phase, float depth) { return 1 + depth * (getFmEnvelopeValueAtPhase(phase) - 1); }
