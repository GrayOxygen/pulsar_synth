...
void PulsarSynthVoice::saveSnapShot() {
  ...
  snapShot.dutyCycleCluster =
      getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate, commonVoiceState->kEnvIndexWrapParam->load()), commonVoiceState->dutyCycleClusterDepthParam->load());
...
void PulsarSynthVoice::changeStage() {
  ...
  case why::PulsarStateEnum::IntraSilence: // The silence stage of pulsar has now been completed
    ...
    envIndex = envIndex + commonVoiceState->kEnvIndexWrapParam->load();
...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  const float pan = getCurrentPan(envTimelinePhase(envIndex, kPanEnvTimelineRate, commonVoiceState->kEnvIndexWrapParam->load()), commonVoiceState->panDepthParam->load());
  if (maskPass) {
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  float amp = calcAmEnvelopeInterpolation(envTimelinePhase(envIndex, kAmEnvTimelineRate, commonVoiceState->kEnvIndexWrapParam->load()), commonVoiceState->amEnvDepthParam->load());
  double fmSlice = calcFmEnvelopeInterpolation(envTimelinePhase(envIndex, kFmEnvTimelineRate, commonVoiceState->kEnvIndexWrapParam->load()), commonVoiceState->fmEnvelopeDepthParam->load());
