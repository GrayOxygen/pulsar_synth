...
void PulsarSynthVoice::mappingParams(const juce::AudioProcessorValueTreeState &apvts) {
  ...
  // async grain cloud
  commonVoiceSate->cloudDensityParam = apvts.getRawParameterValue(why::ParameterID::cloudDensity);
  commonVoiceSate->cloudLevelParam = apvts.getRawParameterValue(why::ParameterID::cloudLevel);
  commonVoiceSate->cloudPitchSpreadParam = apvts.getRawParameterValue(why::ParameterID::cloudPitchSpread);
...
void PulsarSynthVoice::mappingOneParam(const juce::AudioProcessorValueTreeState &apvts, juce::String parameterID, float newValue) {
  ...
  // async grain cloud
  if (parameterID == why::ParameterID::cloudDensity && commonVoiceSate->cloudDensityParam != nullptr) {
    commonVoiceSate->cloudDensityParam->store(newValue);
  }
  if (parameterID == why::ParameterID::cloudLevel && commonVoiceSate->cloudLevelParam != nullptr) {
    commonVoiceSate->cloudLevelParam->store(newValue);
  }
  if (parameterID == why::ParameterID::cloudPitchSpread && commonVoiceSate->cloudPitchSpreadParam != nullptr) {
    commonVoiceSate->cloudPitchSpreadParam->store(newValue);
  }
  // masking
...
float PulsarSynthVoice::processSample() {
  ...
  // async grain cloud：独立于pulsar train的随机触发调度——密度=每秒grain数，
  // 触发间隔在平均值±50%内均匀抖动(异步云感)，与同步pulsar train共用同一个granular引擎/grain池
  const float cloudDensity = commonVoiceSate->cloudDensityParam != nullptr ? commonVoiceSate->cloudDensityParam->load() : 0.0f;
  if (cloudDensity > 0.01f) {
    if (--cloudCountdownSamples <= 0) {
      spawnCloudGrain();
      double meanIntervalSamples = getSampleRate() / static_cast<double>(cloudDensity);
      double jitter = 0.5 + juce::Random::getSystemRandom().nextDouble(); // 0.5x ~ 1.5x
      cloudCountdownSamples = std::max(1, static_cast<int>(meanIntervalSamples * jitter));
    }
  }
...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, 1.0f, 1.0f, false, 1.0f);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, 1.0f, 1.0f, false, 1.0f);
    spawnGrain(snapShot.fundamentalFreq * 5.0f, 1.0f, 1.0f, false, 1.0f);
    spawnGrain(snapShot.fundamentalFreq * 7.0f, 1.0f, 1.0f, false, 1.0f);

    // double phase = std::fmod(static_cast<float>(fmLatch * snapShot.fundamentalFreq) / juce::MathConstants<float>::twoPi, 1.0f);
    ...
    // spawnGrain(snapShot.fundamentalFreq * 2.0f, active / 8.0f / 2.0f, sine, false, triangle);
    // spawnGrain(snapShot.fundamentalFreq * 3.0f, r * active / 8.0f, sine, false, sine);
    // spawnGrain(snapShot.fundamentalFreq * 4.0f, active / 8.0f / 4.0f, sine, false, saw);
    // if (r < 0.3) {
    // } else if (r < 0.7) {
    // } else {
    // }
...
// 异步云grain：触发时刻由密度调度器决定(与pulsar period无关)，围绕当前基频随机失谐(±spread semitones均匀分布)、
// 随机时长(2~8个波形周期)；fund=false保证不覆盖voice级波表扫描/LFO速率latch，云与train和平共存
void PulsarSynthVoice::spawnCloudGrain() {
  if (commonVoiceSate->cloudLevelParam == nullptr || commonVoiceSate->cloudPitchSpreadParam == nullptr)
    return;
  const float level = commonVoiceSate->cloudLevelParam->load();
  // 听不见的grain不入池(与spawnGrain同理：白占slot还拉低√N归一化)
  if (level <= 1.0e-4f)
    return;

  auto &rng = juce::Random::getSystemRandom();
  const float spread = commonVoiceSate->cloudPitchSpreadParam->load();
  const double semis = (rng.nextDouble() * 2.0 - 1.0) * static_cast<double>(spread);
  const double freq = static_cast<double>(snapShot.fundamentalFreq) * std::pow(2.0, semis / 12.0);
  if (!(freq > 0.0))
    return;

  const double sr = getSampleRate();
  const double cycleSamples = std::round(sr / freq);
  if (cycleSamples <= 0.0)
    return;
  // 2~8个波形周期：≥2保证renderSample走COLA Hann窗路径(平滑云粒)；上限防止低频+向下散布时grain过长把池占满
  const int durCycles = 2 + rng.nextInt(7);
  int playbackSamples = std::max(1, static_cast<int>(cycleSamples * durCycles));
  playbackSamples = std::min(playbackSamples, static_cast<int>(sr * 2.0));

  GranularEngine::SpawnParams params;
  params.startPhase = 0.0;
  params.phaseInc = 1.0 / cycleSamples;
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  scanFreq *= (1.0f + std::pow(2.0, static_cast<double>(detuneCents) / 1200.0));
  ...
  // 厚度/共振感自动演化，不需要动任何旋钮
  double durCycles = static_cast<double>(snapShot.dutyCycleCluster) * length * (1.0 + 0.2 * driftWt);
...
// granular核心引擎完成所有grain的overlap-add渲染与归一化
float PulsarSynthVoice::calcActualPulse() { return granular.renderSample(getSampleRate()); }
