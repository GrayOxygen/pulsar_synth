...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  float freqModPhase = std::fmod(driftPhase, 1.0f);
  float triangle = 1.0f - 4.0f * std::abs(freqModPhase - 0.5f);
  // per-grain失谐必须限制在cents级：之前的(sin+triangle)/(active+2)可达±25%~60%速率差，
  // 重叠的失谐拷贝高频相位先散(√N非相干相加)、低频仍相干(N倍相加)——频谱向低频倾斜变浑；
  // 大失谐的拍频包络过limiter还会产生差频(f1-f2)低频失真产物。
  // 重叠grain各自Latch不同失谐=合唱效果，幅度∝cents范围：±3 cents只留轻微流动感，调大变chorus
  float detuneCents = 3.0f * (std::sin(driftPitch * fmLatch)) / 4.0f;
