...
void PulsarSynthVoice::spawnGrain() {
  ...
  float startPhaseOffset = static_cast<float>(active) / 8.0f;
  float ampRandom = 2.0f * static_cast<float>(active) / 8.0f / 10.0f - 0.1; // [-0.1, 0.1]
  ...
  int samples = (currentStateDurationSampleNum - 1) * ampRandom * static_cast<int>(getCurrentDutyCycleCluster(envPhase, commonVoiceSate->dutyCycleClusterDepthParam->load()));
...
inline float gaussianEnvelope(float t, float sigma = 0.3f) {
  ...
  sigma *= gaussianEnvelope(static_cast<float>(currentStateDurationSampleNum) / static_cast<float>(currentStateDurationSampleNum - 1), 0.5);
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    float completeWindow = gaussianEnvelope(g.windowPhase, 0.37);
