...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * window* g.amp * (1 + std::sin(2.0f * juce::MathConstants<float>::pi * amLfo)); // am modulation
    activeCount++;
    g.phase += g.phaseInc + (std::sin(2.0f * juce::MathConstants<float>::pi * fmLfo) / static_cast<float>(getSampleRate())); // fm modulation
    if (g.phase >= 1.0f)
      g.phase -= 1;
...
float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) {
  ...
  return getAmLfoValueAtPhase(phase) * depth;
}
