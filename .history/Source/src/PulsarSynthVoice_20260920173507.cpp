...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    // cents级失谐产生缓慢拍频=supersaw式厚度，不会像大失谐那样粗糙
        const float unisonDetune = commonVoiceSate->unisonDetune != nullptr ? commonVoiceSate->unisonDetune->load() : 0.0f;

    if (unisonDetune > 0.01f) {
      spawnGrain(snapShot.fundamentalFreq, 0.7f, 1.0f, false, 1.0f, -unisonDetune, pan);
      spawnGrain(snapShot.fundamentalFreq, 0.7f, 1.0f, false, 1.0f, +unisonDetune, pan);
    }
  }
}
