...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, amLatch, 1.0f * fmLatch, false, amLatch + r);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, amLatch, 0.75f * fmLatch, false, std::cos(amLatch * r));
    spawnGrain(snapShot.fundamentalFreq * 4.0f, amLatch, 0.5f * fmLatch, false, std::sin(amLatch) * r);
  }
}
