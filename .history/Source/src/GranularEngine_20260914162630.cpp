...
void GranularEngine::spawn(const SpawnParams &p) {
  ...
  // 就是听到的噪/crack的主源之一
  if (p.fund) {
    // wavetable scanning: one full table sweep per 44100 pulsar periods (matches envPhase timeline),
    // direction sign preserved so ping-pong isn't reset
    wtScanInc = (wtScanInc < 0.0 ? -1.0 : 1.0) * p.baseFreq / (44100.0 * p.sampleRate);
...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    double completeWindow = overlapCycles >= 2.0f ? 0.5 - 0.5 * std::cos(juce::MathConstants<double>::twoPi * g.phase) : cycleEdgeWindow(static_cast<float>(g.phase));
