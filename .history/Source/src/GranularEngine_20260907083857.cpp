...
void GranularEngine::spawn(const SpawnParams &p) {
  ...
  if (slot < 0) {
    ...
    for (int i = 0; i < MAX_GRAINS; ++i) {
      ...
      // 生命周期进度：0 = 刚出生，1 = 快结束
      const float lifeProgress = g.totalSamples > 0 ? juce::jlimit(0.0f, 1.0f, static_cast<float>(g.remainSamples) / static_cast<float>(g.totalSamples)) : 1.0f;
      ...
      // 越高越应该被替换
      const float score = nearEnd * 0.60f + (1.0f - envelope) * 0.25f + ageBonus * 0.10f + randomJitter * 0.05f;
