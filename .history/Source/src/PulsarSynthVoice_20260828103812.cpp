...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  int r = juce::Random::getSystemRandom().nextInt(100);
  ...
  if (maskPass) {
    ...
    // double fmEnvelope = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    double fmSlice = calcFmEnvelopeInterpolation(envIndex / 2048.0f, commonVoiceSate->fmEnvelopeDepthParam->load());
    spawnGrain(snapShot.fundamentalFreq * 2.0f, 0.75f, 0.3f);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, 0.5f, 0.1f);
    spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.3f, 0.2f);
    spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.1f, 0.3f);
    spawnGrain(snapShot.fundamentalFreq * 6.0f, 0.01f, 0.02f);
    spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.05f, 0.1f);
    if (r % 5 == 0) {
      spawnGrain(snapShot.fundamentalFreq * 2.0f * fmSlice, 0.01f, 0.1f);
      spawnGrain(snapShot.fundamentalFreq * 3.0f * fmSlice, 0.05f, 0.2);
      spawnGrain(snapShot.fundamentalFreq * 4.0f * fmSlice, 0.02f, 0.3);
    }
    if (r % 5 == 1) {
      spawnGrain(snapShot.fundamentalFreq * 5.0f * fmSlice, 0.05f, 0.3);
      spawnGrain(snapShot.fundamentalFreq * 6.0f * fmSlice, 0.01f, 0.02f);
      spawnGrain(snapShot.fundamentalFreq * 7.0f * fmSlice, 0.02f ,0.01f);
    }
    if (r % 5 == 2) {
      spawnGrain(snapShot.fundamentalFreq * 8.0f * fmSlice, 0.01f, 0.5);
      spawnGrain(snapShot.fundamentalFreq * 9.0f * fmSlice, 0.02f, 0.1);
      spawnGrain(snapShot.fundamentalFreq * 10.0f * fmSlice, 0.05f, 0.7);
    }
    if (r % 5 == 3) {
      spawnGrain(snapShot.fundamentalFreq * 11.0f * fmSlice, 0.01f, 0.7);
      spawnGrain(snapShot.fundamentalFreq * 12.0f * fmSlice, 0.05f, 0.3);
    }
    if (r % 5 == 4) {
      spawnGrain(snapShot.fundamentalFreq * 13.0f * fmSlice, 0.03f, 0.1);
      spawnGrain(snapShot.fundamentalFreq * 14.0f * fmSlice, 0.02f, 0.1);
    }
  }
}
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length) {
  ...
  double trainPhaseInc = static_cast<double>(baseFreq / (snapShot.trainDutyCycleLenParam * getSampleRate()));
  ...
  if (slot >= 0) {
    waveformGrains[slot] = {startPhase, phaseInc, lfoPhase, lfoPhaseInc, true, playbackSamples, playbackSamples, delaySamples, 1.0f, 0, amp, static_cast<float>(baseFreq), wtPos, wtPosInc};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double completeWindow2 = cycleEdgeWindow(static_cast<float>(g.phase), 0.1f);
    ...
    grainSum += raw * g.amp * pulsaretEnvelope * completeWindow * adsrWindow;
  ...
  // waveformSample = static_cast<float>(grainSum) * 0.9f;
