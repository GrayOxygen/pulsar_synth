...
inline float envTimelinePhase(int envIndex, double rate, int kEnvIndexWrap) { return static_cast<float>(std::fmod(static_cast<double>(envIndex) * rate / (kEnvLoopPeriods * kEnvIndexWrap), 1.0)); }
} // namespace
...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    if (unisonDetune > 0.01f) {
      spawnGrain(snapShot.fundamentalFreq, 0.2f * h2, 1.0f, false, 1.0f, -unisonDetune, pan);
