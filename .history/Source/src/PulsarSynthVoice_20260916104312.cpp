...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  double durCycles = static_cast<double>(snapShot.dutyCycleCluster) * (1 + r) * length;
  // 实际duration length
  double playbackSamples = static_cast<double>(durCycles * periodSamples);

  // 一个 pulsaret 在 d 时间内完整扫完一次 waveform
  // detune调节
  double detuneRatio = std::pow(2.0, extraDetuneCents / 1200.0);
  double phaseInc = 1.0f * (1 + active) * static_cast<int>(fmSlice) * detuneRatio / static_cast<double>(playbackSamples);

  amp = static_cast<float>(amp / std::max(1.0, durCycles * 0.5));

  //  只提供trigger时刻算好的参数
  GranularEngine::SpawnParams params;
  params.startPhase = active * r;
  params.phaseInc = phaseInc;
  params.playbackSamples = playbackSamples;
  params.delaySamples = periodSamples * (active + r) / 2.0f;
  params.amp = amp;
