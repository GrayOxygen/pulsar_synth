...
void PulsarSynthVoice::spawnGrain() {
  ...
  double baseFreq = snapShot.pulsarFreq;
  ...
  for (auto &g : waveformGrains) {
    ...
    g.lfoPhaseInc = waveformGrains[lastGrainIndex].lfoPhaseInc;
    g.phase = waveformGrains[lastGrainIndex].phase;
  }

  waveformGrains[waveformGrainWriteIdx] = {lastPhase, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, 0, phaseInc, wtPos};
