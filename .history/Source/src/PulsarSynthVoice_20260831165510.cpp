...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  float freqModPhase = std::fmod(fmLatch * driftPhase, 1.0f);
  float triangle = 1.0f - 4.0f * std::abs(freqModPhase - 0.5f);
  scanFreq *= (1.0f + (std::sin(baseFreq * fmLatch) * driftPitch + triangle) / (active + 2.0f));
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  return activeCount <= 0 ? 0.0f : static_cast<float>(grainSum / std::sqrt(static_cast<double>(activeCount)));
}
