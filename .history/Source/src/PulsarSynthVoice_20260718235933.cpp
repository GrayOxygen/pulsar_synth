...
void PulsarSynthVoice::spawnGrain() {
  ...
  int delaySamples = static_cast<int>(currentStateDurationSampleNum / 4) * active;
  delaySamples = 0;
  startPhaseOffset = 0;
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  return (0.5f + (2.0f * getFmLfoValueAtPhase(phase) * depth - 1.0f) / 2.0f) * (1 + getCellActiveCount() * 0.1f / 8.0f); // [0, 1]
}

float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) {
  return (0.5f + (2.0f * getAmLfoValueAtPhase(phase) * depth - 1.0f) / 2.0f) * (1 + getCellActiveCount() * 0.1f / 8.0f); // [0, 1]
}
