...
void PulsarSynthVoice::spawnGrain() {
  ...
  // NuPG式grain模型(GrainBuf: pos=0, dur=1/formantFreq)：grain = 整数个完整波形周期。
  // 先由调制后的速率确定单个波形周期时长(rate升高->周期变短，duty涌现变化，而非截断波形)，
  // 再反推grain总时长 = cluster个完整周期；take整后重算phaseInc，保证恰好在整数周期处结束，
  // 首尾同点天然闭合，不存在相位不连续问题
  double rate = fmMult * fmLatch;
  double cycleSamples = rate > 0.0 ? currentStateDurationSampleNum / rate : static_cast<double>(currentStateDurationSampleNum); // 单个波形周期的样本数
  double cycles = std::max(1.0, std::round(snapShot.dutyCycleCluster)); // grain内包含的完整周期数(取整)
  int samples = std::max(1, static_cast<int>(std::round(cycleSamples * cycles)));
  double phaseInc = cycles / static_cast<double>(samples);
  ...
  // 全局LFO速率跟随最新grain时长(一个grain生命周期对应一个LFO周期)；fm/amLatch已在上方采样
  ...
  // 首尾天然同点，不再需要相位延续
  waveformGrains[waveformGrainWriteIdx] = {0.0, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, lfoPhase, lfoPhaseInc, wtPos, wtPosInc};
