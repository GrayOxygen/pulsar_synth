...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, 0.75f, 0.1f * r);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, 0.5f, 0.95f * r);
    spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.3f, 0.5f * r);
    spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.1f, 0.3f * r);
    spawnGrain(snapShot.fundamentalFreq * 6.0f, 0.01f, (0.35f + r) / 2.f);
    spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.05f, (0.3f + r) / 2.f);
    // if (r % 3 == 0) {
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length) {
  ...
  scanFreq *= std::pow(2.0, static_cast<double>(driftPitch * std::sin(fmLatch) * r) * 12.0 / 1200.0);
