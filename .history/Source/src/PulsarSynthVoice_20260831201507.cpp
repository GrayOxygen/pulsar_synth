...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    double phase = std::fmod(static_cast<float>(lfoPhase) / juce::MathConstants<float>::twoPi, 1.0f);
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, (amLatch + r) * active / 8.0f / 2.0f, sine, false, triangle);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, 1.0f * r * active / 8.0f, sine, false, sine);
    spawnGrain(snapShot.fundamentalFreq * 4.0f, amLatch * active / 8.0f, sine, false, saw);
  }
}
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  float detuneCents = 3.0f * (std::tanh(driftPitch * fmLatch)) / 2.0f;
  ...
  wtScanInc = (wtScanInc < 0.0 ? -1.0 : 1.0) * baseFreq / (2048.0 * getSampleRate()) * (1.0f + 0.1f * (std::cos(driftWt * fmLatch)));
