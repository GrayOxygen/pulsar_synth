...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  float targetRatio = juce::jlimit(0.794f, 1.26f, static_cast<float>(fmLatch));
  ...
  // (matches envPhase timeline), direction sign preserved so ping-pong isn't reset: * (1.0f + 0.1f * (std::cos(fmLatch))
  wtScanInc = (wtScanInc < 0.0 ? -1.0 : 1.0) * baseFreq / (2048.0 * getSampleRate()) * (1.0f + 0.1f * (std::cos(baseFreq * fmLatch)));
  ...
  // 注意：绝不能在这里动envIndex——它是2048步宏观时间线(每trigger只该+1)，
  // 之前的envIndex*=(1+rateMod)每次spawn几何倍增，十几个period就wrap一圈，
  // 整个包络演化被压成几秒的短循环
