...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq, amLatch, 1.0f, true, fmLatch);
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  amp *= amFactor;
  amp *= (1.0f + std::sin(baseFreq * amLatch));
  ...
  phaseInc *= (1.0f + std::sin(baseFreq * fmLatch));
  float phase = baseFreq * fmLatch;
  float normalizedPhase = std::fmod(phase / juce::MathConstants<float>::twoPi, 1.0f); 
  float triangle = 1.0f - 4.0f * std::abs(normalizedPhase - 0.5f);
  
  phaseInc *= (1.0f + triangle);
