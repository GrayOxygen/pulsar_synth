...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, 1.0f, 1.0f, true, 1.0f);
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  // scanFreq *= std::pow(2.0, static_cast<double>(driftPitch) * 0.5f / 1200.0); // ±12 cents微失谐漂移
  // amp *= 1.0f + driftAmp * 0.05f; // ±5%音量呼吸感
  // fmLatch已是音高ratio(2^(semi·depth/12))，但直接乘有两个问题：
  // 1) canvas极值会把rate推到过大/过小——夹到±4半音(0.794~1.26)，可闻但不刺耳；
  // 2) 每个grain硬Latch一个新ratio，相邻grain音高台阶跳变(zipper颤音)——
  //    用一阶slew(每次spawn向目标走15%)让音高在grain之间连续滑动，portamento感
  // // float targetRatio = juce::jlimit(0.794f, 1.26f, static_cast<float>(driftPitch * fmLatch));
  // // smoothedFmRatio += (targetRatio - smoothedFmRatio) * 0.15f;
  // // scanFreq *= smoothedFmRatio * rateMod;
  double saw = (2.0f * fmLatch - 1.0f) * 0.5f;
  // scanFreq *= (1.0f + saw * driftPitch * 0.1f);
  ...
  int playbackSamples = static_cast<int>(cycleSamples * durCycles);
  ...
  // (matches envPhase timeline), direction sign preserved so ping-pong isn't reset: * (1.0f + r * (std::cos(baseFreq * fmLatch)))
  wtScanInc = (wtScanInc < 0.0 ? -1.0 : 1.0) * baseFreq / (2048.0 * getSampleRate()) * (1.0f + r * (std::cos(baseFreq * fmLatch)));
  float wtPos = static_cast<float>(wtScanPos);
  
  // follow the scan freq, latch：phaseInc的cycleSamples已含fmLatch调制，再乘一次会双重应用，
  // 低ratio时LFO被自己的输出拖慢甚至停滑(正反馈)，高ratio时越转越快
  lfoPhaseInc = phaseInc;
  // lfoPhaseInc = phaseInc * (1.0f + saw);
  ...
  if (slot >= 0) {
    waveformGrains[slot] = {
        startPhase, phaseInc, lfoPhase, lfoPhaseInc, true, playbackSamples, playbackSamples, delaySamples, 1.0f, 0, amp, static_cast<float>(baseFreq), wtPos, static_cast<float>(wtScanInc)};
