...
void PulsarSynthVoice::processBlockSamples(juce::AudioSampleBuffer &outputBuffer, int startSample, int numSamples) {
  ...
  for (int sampleIndex = startSample; sampleIndex < (startSample + numSamples); sampleIndex++) {
    ...
    for (int chan = 0; chan < numChannels; chan++) {
      ...
      const float s = juce::dsp::FastMathApproximations::tanh(dcBlocker.process(pulseReadPtr[i])) * outputGain;
...
void PulsarSynthVoice::spawnGrain() {
  ...
  float caPitchOffset = perlinMod; //  perlin
  float startPhaseOffset = 0.5f * static_cast<float>(active) / 8.0f;
  float scale = 0.7f + 0.3f * static_cast<float>(active) / 8.0f;
  caPitchOffset = 1.0f;
  ...
  // float amp = calcAmEnvelopeInterpolation(envPhase, commonVoiceSate->amEnvDepthParam->load()) * (0.9f + 0.1f * static_cast<float>(active) / 8.0f);
  float amp = calcAmEnvelopeInterpolation(envPhase, commonVoiceSate->amEnvDepthParam->load());
  // int delaySamples = static_cast<int>(currentStateDurationSampleNum / 4) * active; // 听起来卡顿

  scale = 1.0f;
  startPhaseOffset = 0.0f;

  waveformGrains[waveformGrainWriteIdx] = {startPhaseOffset, phaseInc, true, samples, samples, scale, 0, amp};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * windowInCurrentStage * getPulseFadeGain(); // am modulation
    // grainSum += raw * completeWindow * getPulseFadeGain() * windowInCurrentStage ; // am modulation
    grainSum = juce::dsp::FastMathApproximations::tanh(grainSum * amLfo * g.scale * g.amp);
    activeCount++;
