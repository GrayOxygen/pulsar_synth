...
void GranularEngine::spawn(const SpawnParams &p) {
  ...
  // 就是听到的噪/crack的主源之一
  if (p.fund) {
    // wavetable scanning: one full table sweep per 96000 pulsar periods (matches envPhase timeline),
    // direction sign preserved so ping-pong isn't reset
    wtScanInc = (wtScanInc < 0.0 ? -1.0 : 1.0) * p.baseFreq / (96000.0 * p.sampleRate);
