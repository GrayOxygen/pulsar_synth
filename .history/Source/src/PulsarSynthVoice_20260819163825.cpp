...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double lfoAmount = getCurrentDutyCycleRatio(g.phase, commonVoiceSate->dutyCycleRatioDepthParam->load());

    g.phase += g.phaseInc * lfoAmount;
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
