...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, amLatch, 1.0f * fmLatch, false, amLatch);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, amLatch, 0.75f * fmLatch, false, std::cos(amLatch));
    spawnGrain(snapShot.fundamentalFreq * 4.0f, amLatch, 0.5f * fmLatch, false, std::sin(amLatch));
    // spawnGrain(snapShot.fundamentalFreq * fmLatch, amLatch * r, 3.0f, false);
    // spawnGrain(snapShot.fundamentalFreq * 2.0f, 0.75f * amLatch, 0.95f * fmLatch, false);
    // spawnGrain(snapShot.fundamentalFreq * 3.0f, 0.5f * amLatch, (0.75f + fmLatch) / 3.0f, false);
    // spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.1f * std::sin(amLatch), 0.5f * fmLatch, false);
    // spawnGrain(snapShot.fundamentalFreq * 5.0f, (0.9f + amLatch) / 2.0f, 0.15f * fmLatch, false);
    // spawnGrain(snapShot.fundamentalFreq * 6.0f, (0.51f + amLatch) / 3.0f, (0.75f + fmLatch) / 2.f, false);
    // spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.85f * amLatch, (0.9f + fmLatch) / 2.f, false);
    // if (r % 3 == 0) {
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  amp *= 1.0f + static_cast<float>(std::sin(baseFreq * amLatch));
  ...
  double tempLfoPhase = static_cast<double>(baseFreq) * fmLatch;
  float normalizedPhase = std::fmod(static_cast<float>(tempLfoPhase) / juce::MathConstants<float>::twoPi, 1.0f);
  float triangle = 1.0f - 4.0f * std::abs(normalizedPhase - 0.5f);

  phaseInc *= (1.0f + triangle * rateMod);
  ...
  // NuPG的LFSaw.ar(ffreq * fmRatio)：调制LFO频率跟随fundamental(fmRatio=1)，
  // 相位全局自由运行(processSample逐sample推进)，每次trigger在maskGatedSpawn处Latch采样
  if (fund)
    lfoPhaseInc = getSampleRate() > 0.0 ? baseFreq / getSampleRate() : 0.0;
