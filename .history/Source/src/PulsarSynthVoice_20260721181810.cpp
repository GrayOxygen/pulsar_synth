...
void PulsarSynthVoice::refreshPulsarInSnapShot() {
  ...
  snapShot.pulsarDutyCycleSamples = snapShot.dutyCycleRatio * snapShot.pulsarPeriodTime * sr;
  snapShot.pulsarIntraSilenceSamples = snapShot.pulsarPeriodTime * sr - snapShot.pulsarDutyCycleSamples;
}
...
float PulsarSynthVoice::calSampleByState(bool passMaskFlag, bool existMask) {
  ...
  case why::PulsarStateEnum::InterTrainSilence:
    // 不再spawn新grain，但让活跃grain的尾巴自然播完(hann窗衰减到0)，避免train边界硬切产生click
...
void PulsarSynthVoice::spawnGrain() {
  ...
  // int samples = static_cast<int>(currentStateDurationSampleNum * snapShot.dutyCycleCluster * (1.0f + 1.0f * active / 2.0f));
  int samples = static_cast<int>(currentStateDurationSampleNum * snapShot.dutyCycleCluster);
  // 一个pulse(duty段)恰好扫完一个完整波形周期：phaseInc = 1/dutySamples。
  // grain结束时phase正好闭合回0，相位延续自动成立，每个pulse波形/振幅一致；cluster>1时grain扫完cluster个完整周期，同样闭合。
  double phaseInc = currentStateDurationSampleNum > 0 ? fmMult / static_cast<double>(currentStateDurationSampleNum) : 0.0;
  ...
  // 下一个train从0重新扫
  float trainLen = std::max(1.0f, snapShot.trainDutyCycleLenParam);
  float wtPos = static_cast<float>(periodsDoneInTrain) / trainLen;
  float wtPosInc = static_cast<float>(snapShot.fundamentalFreq / (trainLen * getSampleRate()));
  ...
  // 相位延续：新grain从上一个spawn的grain的当前相位继续扫描波形，保证跨pulse的波形连续（如sine前半周期 -> 后半周期）
  // 注意：若上一个grain还活着，spawn发生在本sample的calcActualPulse之前，两者随后逐样本同步推进(同inc时相位恒等)，不会有一个sample的错位
  int lastGrainIndex = (waveformGrainWriteIdx + MAX_WAVEFORM_GRAINS - 1) % MAX_WAVEFORM_GRAINS;
  double lastPhase = waveformGrains[lastGrainIndex].phase;

  // 全局LFO速率跟随最新grain的时长(一个grain生命周期对应一个LFO扫描周期)，但相位全局连续
  // lfoPhaseInc = samples > 0 ? 1.0 / static_cast<double>(samples) : 0.0;

  waveformGrains[waveformGrainWriteIdx] = {lastPhase, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, lfoPhase, lfoPhaseInc, wtPos, wtPosInc};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  int activeCount = 0;
  double windowSum = 0.0; // 各grain的hann窗值之和：连续的"有效重叠数"，用于无跳变归一化
  double s = 0;
  ...
  for (auto &g : waveformGrains) {
    ...
    double fmLfo = calcFmLfoInterpolation(static_cast<float>(g.phase), commonVoiceSate->fmLfoDepthParam->load());
    double amLfo = calcAmLfoInterpolation(static_cast<float>(g.phase), commonVoiceSate->amLfoDepthParam->load());
    // g.lfoPhase += g.lfoPhaseInc;
    // if (g.lfoPhase >= 1.0)
    //   g.lfoPhase -= 1.0;
    ...
    grainSum += raw * completeWindow * g.amp * fmLfo; // am modulation
    windowSum += completeWindow;
    activeCount++;
  ...
  // 归一化用hann窗加权和(windowSum)而非整数activeCount：
  // 整数count在grain生灭瞬间跳变(尤其1->2时分母突增62%)，正在发声的grain音量瞬间跌落产生咔哒；
  // windowSum随grain的hann窗从0平滑升降，分母连续无跳变；max(1,·)避免低重叠时放大窗边缘信号
  waveformSample = activeCount > 0 ? grainSum / std::pow(std::max(1.0, windowSum), 0.7) : 0.0f;
