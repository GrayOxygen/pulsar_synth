...
void PulsarSynthVoice::spawnGrain() {
  ...
  // NuPG式相位相干：grain准时在period边界触发、从phase 0开始——重叠时同相叠加变厚而不是echo；
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    // 只用hann窗(NuPG语义)：之前额外乘grainAdsrShape(g.phase,...)用的是每周期循环的0~1波形相位，
    // 变成基频斩波器且g.amp被误用作sustain，磨掉攻击感、变化不剧烈
  ...
  // N重叠grain直接相加不归一化(GrainBuf mul:0.9)，重叠即增益、越叠越厚(formant共振感)；
