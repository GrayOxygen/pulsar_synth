...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  double ampSum = 0.0; // 活跃grain的|amp|线性和(相干上限)：用于归一化
  int activeCount = 0;
  ...
  for (auto &g : grains) {
    ...
    activeCount++;
    ampSum += static_cast<double>(g.amp);
  ...
  // 归一化用|amp|线性和(相干上限)而不是功率√(Σamp²)：OPulWM的重叠拷贝同源同相位、
  // COLA窗和≈N/2×波形，是"幅度相加"而不是"功率相加"——功率归一化只压回1/√N，
  // 残余√N/2的过冲随cluster增大持续推进tanh削波=互调失真(听感=噪/harsh)。
  // 按ΣA归一化后N个等幅相干grain输出稳定在~0.5，重叠再多也不削波；
  // 安静的谐波/unison grain只按自身电平计权，生灭几乎不动增益。
  // activeCount==0时冻结，避免下个grain出生时增益从错误值滑动
  if (activeCount > 0) {
    const float targetPower = static_cast<float>(ampSum);
  ...
  const double norm = 1.0 / static_cast<double>(smoothedGrainPower);
