...
void PulsarSynthVoice::spawnGrain() {
  ...
  waveformGrains[waveformGrainWriteIdx] = {startPhaseOffset, phaseInc, true, samples, samples, static_cast<int>(currentStateDurationSampleNum * (active / 8.0f)), 0, amp};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * window * g.amp * amLfo; // am modulation
    activeCount++;
