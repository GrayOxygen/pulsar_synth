...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  // wavetable scanning：不再per-grain冻结/各自滑动——扫描位置改为voice级自由运行(wtScanPos)，
  // calcActualPulse里逐sample推进，所有重叠grain同一时刻读同一个位置：
  // 帧morph连续glide且grain间完全相干，grain生灭不会带来帧位置跳变。
  // 扫描速率在trigger时Latch：2048个pulsar period恰好扫完整个波表(与envPhase时间线一致)
  if (fund && getSampleRate() > 0.0)
    wtScanInc = baseFreq / (2048.0 * getSampleRate());
