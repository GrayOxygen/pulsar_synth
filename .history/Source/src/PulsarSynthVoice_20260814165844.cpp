...
void PulsarSynthVoice::spawnGrain() {
  ...
  // if (r < 0.5) {
  //   scanFreq = scanFreq * (1.0f + std::sin(2.0f * juce::MathConstants<float>::pi * scanFreq)) * perlinMod;
  // }
  ...
  double startPhase = 0;
  if (r < 0.5) {
    delaySamples = static_cast<int>(getSampleRate() / 1000.0f * active);
    startPhase = static_cast<float>(active / 8.0f);
  }
  ...
  double phaseInc = cycleSamples > 0.0 ? (1.0f + active < 3 ? 0 : active / 3.0f) / cycleSamples : 0.0; // 纯播放速率，与包络时长解耦
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    // grainSum += raw * completeWindow * g.amp; // am modulation(已Latch)  * grainAdsrShape(g.phase, 0.2, 0.2, 1, 0.2)
    grainSum += raw * completeWindow * g.amp * grainAdsrShape(g.phase, 0.1, 0.2, g.amp, 0.4);
    double perlinMod = getPerlinModulation();
    double fmLatch = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    double factor = getCurrentDutyCycleRatio(lfoPhase, commonVoiceSate->dutyCycleRatioDepthParam->load());
    g.phase += g.phaseInc * (1.0f + factor*perlinMod) ; // fm modulation(已Latch进phaseInc)
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
