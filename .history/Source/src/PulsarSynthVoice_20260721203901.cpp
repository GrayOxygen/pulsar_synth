...
void PulsarSynthVoice::spawnGrain() {
  ...
  caPitchOffset = 1.0f;
  double fmMult = baseFreq * calcFmEnvelopeInterpolation(envPhase, commonVoiceSate->fmEnvelopeDepthParam->load()) * caPitchOffset;
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double fmLfo = calcFmLfoInterpolation(static_cast<float>(g.phase * 1.0f / snapShot.dutyCycleCluster)), commonVoiceSate->fmLfoDepthParam->load());
    double amLfo = calcAmLfoInterpolation(static_cast<float>(g.phase * 1.0f / snapShot.dutyCycleCluster), commonVoiceSate->amLfoDepthParam->load());
