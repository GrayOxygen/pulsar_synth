...
// per-grain ADSR形状(NuPG式：包络作用范围=单个grain完整寿命)：t为grain窗相位0~1，
...
void PulsarSynthVoice::spawnGrain() {
  ...
  caPitchOffset = 1.0f;
  double fmSlice = calcFmEnvelopeInterpolation(envPhase, commonVoiceSate->fmEnvelopeDepthParam->load()) * caPitchOffset;
  ...
  // 无需整数周期闭合
  double rate = fmSlice;
  double grainBaseSamples = snapShot.pulsarDutyCycleSamples;
  double cycleSamples = rate > 0.0 ? grainBaseSamples / rate : grainBaseSamples; // 单个波形周期的样本数，有可能超过pulsar duty cycle length or even pulsar period length

  double cycleFreq = snapShot.pulsarFreq * rate;
  cycleFreq = std::clamp(cycleFreq, 1.0, 3000.0);
  cycleSamples = std::round(getSampleRate() / cycleFreq);

  // cycleSamples = rate < 0.5 ? std::min(cycleSamples, static_cast<double>(snapShot.pulsarDutyCycleSamples * 2)) : cycleSamples; // 限制最长为500ms一个grain
  ...
  // double durCycles = std::max(0.05, static_cast<double>(snapShot.dutyCycleCluster) * overlapMult * dilation);
  double durCycles = std::max(0.05, static_cast<double>(snapShot.dutyCycleCluster));
  ...
  if (slot >= 0) {
    waveformGrains[slot] = {0.0f, phaseInc, true, playbackSamples, playbackSamples, delaySamples, scale, 0, amp, wtPos, wtPosInc};
    // Latch per-grain ADSR形状：UI参数视为占grain寿命的比例，归一化使a+d+r≤1；
    // a/r钳住下限保证首尾平滑到0(无click)
    {
      auto &ng = waveformGrains[slot];
      float a = juce::jlimit(0.005f, 1.0f, commonVoiceSate->attackParam->load());
      float d = juce::jmax(0.0f, commonVoiceSate->decayParam->load());
      float r = juce::jlimit(0.005f, 1.0f, commonVoiceSate->releaseParam->load());
      float total = a + d + r;
      if (total > 1.0f) {
        a /= total;
        d /= total;
      ...
      ng.envAttack = a;
      ng.envDecay = d;
      ng.envSustain = juce::jlimit(0.0f, 1.0f, commonVoiceSate->sustainParam->load());
      ng.envRelease = r;
    }
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    //   g.windowPhase = 1.0;
    // per-grain ADSR窗(替代Hann)：UI的attack/decay/sustain/release按grain寿命比例生效，
    // 天然适配任意cluster/dilation长度；首尾为0，grain生灭无click
    double adsrWindow = grainAdsrShape(static_cast<float>(g.windowPhase), g.envAttack, g.envDecay, g.envSustain, g.envRelease);
    double completeWindow = gaussianEnvelope(static_cast<float>(g.windowPhase));
    // float activePortion = g.totalSamples / getSampleRate() <= 1.0f ? 1.0f : (1.0f / (g.totalSamples / getSampleRate())); // 30% 有能量
    // float x = g.windowPhase / activePortion;
    // float window;
    // if (x >= 1.0f)
    //   window = 0.0f;
    // else
    //   window = gaussianEnvelope(x, 0.5f);
    ...
    grainSum += raw * completeWindow * adsrWindow * g.amp; // am modulation(已Latch)
    double fmLatch = calcFmLfoInterpolation(static_cast<float>(g.phase), commonVoiceSate->fmLfoDepthParam->load());

    g.phase += g.phaseInc * fmLatch; // fm modulation(已Latch进phaseInc)
    if (g.phase >= 1.0f)
      g.phase -= 1;
  ...
  waveformSample = activeCount <= 0 ? 0.0f : static_cast<float>(1.0f * grainSum / std::sqrt(static_cast<double>(activeCount)));
  // NuPG式：重叠grain直接相加不归一化(GrainBuf mul:0.9)，重叠即增益、越叠越厚(formant共振感)；
  // 电平由输出端limiter/用户增益兜底
  // waveformSample = grainSum * 0.9;
  // 不再乘全局pulsarAdsr：它每stage reset归零，会把跨stage的overlap grain尾巴集体硬切；
  // per-pulsaret包络已由上方per-grain ADSR窗承担(NuPG语义)
  s = waveformSample;
  return s;
}
