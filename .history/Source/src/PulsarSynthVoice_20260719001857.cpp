...
void PulsarSynthVoice::spawnGrain() {
  ...
  float startPhaseOffset = static_cast<float>(active) / 8.0f;                            // [0, 1]* perlinMod
  ...
  float amp = calcAmEnvelopeInterpolation(envPhase, commonVoiceSate->amEnvDepthParam->load()) * perlinMod * (0.9f + 0.2f * static_cast<float>(active) / 8.0f);
