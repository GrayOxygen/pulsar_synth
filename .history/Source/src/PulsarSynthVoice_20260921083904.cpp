...
void PulsarSynthVoice::saveSnapShot() {
  ...
  snapShot.dutyCycleCluster = getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate,commonVoiceState->kEnvIndexWrapParam.load()), commonVoiceSate->dutyCycleClusterDepthParam->load());
...
void PulsarSynthVoice::refreshPulsarInSnapShot() {
  ...
  snapShot.dutyCycleRatio = 1.0f;
  snapShot.dutyCycleCluster = getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate,commonVoiceSate->kEnvIndexWrapParam.load()), commonVoiceSate->dutyCycleClusterDepthParam->load());
  snapShot.dutyCycleTime = snapShot.dutyCycleRatio * snapShot.pulsarPeriodTime;
  snapShot.pulsarSilenceTime = (1 - snapShot.dutyCycleRatio) * snapShot.pulsarPeriodTime;
  snapShot.pulsarFreq = 1.0 / snapShot.dutyCycleTime;
...
void PulsarSynthVoice::parameterChanged(juce::AudioProcessorValueTreeState &apvts, juce::String parameterID, float newValue, bool &isGeneratedStochasticMask) {
  ...
  if (parameterID == why::ParameterID::dutyCycleClusterDepth) {
    if (snapShot.dutyCycleCluster != getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate,commonVoiceState->kEnvIndexWrapParam.load()), commonVoiceSate->dutyCycleClusterDepthParam->load())) {
