...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * g.amp * completeWindow * completeWindow2 * adsrWindow;
  ...
  waveformSample = activeCount <= 0 ? 0.0f : static_cast<float>(grainSum / std::sqrt(static_cast<double>(smoothedActiveCount)));
  ...
  // per-pulsaret包络已由上方per-grain ADSR窗承担
  s = waveformSample;
  return s;
}
