...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    if (r % 5 == 4) {
      ...
      spawnGrain(snapShot.fundamentalFreq * 8.0f, 0.5f);
      spawnGrain(snapShot.fundamentalFreq * 9.0f, 0.25f);
    }
  }
}
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor) {
  ...
  double envPhase = 1.0f * envIndex * adsrWindow / 2048.0f;
  ...
  if (slot >= 0) {
    waveformGrains[slot] = {startPhase, phaseInc, lfoPhase, lfoPhaseInc, true, playbackSamples, playbackSamples, delaySamples, 1.0f, 0, amp, 0, wtPos, wtPosInc};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * g.amp * pulsaretEnvelope * completeWindow * completeWindow2;
