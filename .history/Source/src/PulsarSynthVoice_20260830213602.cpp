...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  float normalizedPhase = std::fmod(static_cast<float>(lfoPhase) * fmLatch / juce::MathConstants<float>::twoPi, 1.0f);
  float triangle = 1.0f - 4.0f * std::abs(normalizedPhase - 0.5f);
  // scanFreq *= (1.0f + std::sin(fmLatch * juce::MathConstants<float>::twoPi));
  ...
  // scanning single pulsar waveform 2048 times only reach 1/2048 in wt
  // float wtPosInc = phaseInc / commonVoiceSate->pgWavetableNumFrames.load();
  float wtPosInc = phaseInc;
  // float wtPosInc = static_cast<float>(baseFreq / (2048.0 * getSampleRate()));
