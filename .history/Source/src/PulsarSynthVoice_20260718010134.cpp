...
float PulsarSynthVoice::getPulseFadeGain() const {
  ...
  if (N == 1)
    return 1.0f;
  if (pos == N - 1) {
    DBG("pos=" << hasPassedSampleNumInsideTrain << " currentStateDurationSampleNum=" << currentStateDurationSampleNum);
  }
  return gaussianEnvelope(static_cast<float>(pos) / static_cast<float>(N), 0.5);
}
