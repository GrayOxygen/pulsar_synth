...
void PulsarSynthVoice::spawnGrain() {
  ...
  // startPhase = r > 0.7 ? active / 8.0f : 0.0f;
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * completeWindow * adsrWindow * g.amp;
...
float PulsarSynthVoice::getCurrentPan(float phase, float depth) {
  ...
  // float yMin = commonVoiceSate->panEnvelopeYMin.load();
  // float yMax = commonVoiceSate->panEnvelopeYMax.load();
...
float PulsarSynthVoice::getPanEnvelopeValueAtPhase(float phase) const {
  return getEnvelopeValueAtPhase(commonVoiceSate->panEnvelopeData, phase, commonVoiceSate->panEnvelopeScale.load());
}
