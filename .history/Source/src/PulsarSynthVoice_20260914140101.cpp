...
// ============================ envelope时间线去锁定 ============================
// 各调制包络的时间线速率比(基准=envIndex/441，每个pulsar period前进一步)：
...
constexpr int kEnvIndexWrap = 441 * 4096;
inline float envTimelinePhase(int envIndex, double rate) { return static_cast<float>(std::fmod(static_cast<double>(envIndex) * rate / 441.0, 1.0)); }
} // namespace
