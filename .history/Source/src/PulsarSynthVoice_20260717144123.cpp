...
void PulsarSynthVoice::spawnGrain() {
  ...
  for (int j = 0; j < 8; ++j)
    active += (caState >> j) & 1;
  float caPitchOffset = (0.9f + 0.3f * (static_cast<float>(active) / 8.0f)) * getPerlinPrfModulation();
  caPitchOffset = 1;
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    float completeWindow = gaussianEnvelope(g.windowPhase, 0.35);
    if (g.remainSamples <= 1)
      completeWindow = 0.0f;
    // float pulsarWindowPhase = fmod(static_cast<float>(currentStateDurationSampleNum - g.remainSamples), static_cast<float>(g.remainSamples % currentStateDurationSampleNum - 1));
    // float pulsarWindow = gaussianEnvelope(pulsarWindowPhase, 0.5f);
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  float bipolar = getFmLfoValueAtPhase(phase) * 2.0f - 1.0f;
  return (1.0f + bipolar * depth * 0.99); // (0, 2], 防止结果为0
}

float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) {
  float bipolar = getAmLfoValueAtPhase(phase) * 2.0f - 1.0f; // symmetric: [-1, 1]
  return (1.0f + bipolar * depth) / 2.0f;                    // [0, 1]
}
