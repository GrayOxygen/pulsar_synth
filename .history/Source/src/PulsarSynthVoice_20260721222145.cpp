...
void PulsarSynthVoice::spawnGrain() {
  ...
  // grain时长用未clamp的ratio：ratio>1时单个周期长于发射周期，grain跨多个period自然overlap(NuPG式：
  // 等价于formant < fundamental)；ratio<1时与旧行为一致(grainBase = duty阶段样本数)
  double grainBaseSamples = snapShot.dutyCycleRatio * snapShot.pulsarPeriodTime * getSampleRate();
