...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    double pulsaretAMEnvelope = calcAmLfoInterpolation(static_cast<float>(g.windowPhase), commonVoiceSate->amLfoDepthParam->load());
    double pulsaretFMEnvelope = calcFmLfoInterpolation(static_cast<float>(g.windowPhase), commonVoiceSate->amLfoDepthParam->load());
    ...
    g.phase += g.phaseInc * (1 + r * 0.1f + 0.7f * pulsaretFMEnvelope / activeCount);
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
  ...
  wtScanPos += wtScanInc * (1 + EnvelopeCanvas::ENVELOPE_SIZE * commonVoiceSate->unisonWidthParam->load() * (0.5f + r / 2.0f));
  // wtScanPos += wtScanInc;
...
float GranularEngine::getFmLfoValueAtPhase(float phase) const { return getEnvelopeValueAtPhase(commonVoiceSate->fmLfoData, phase, commonVoiceSate->fmLfoScale.load()); }
