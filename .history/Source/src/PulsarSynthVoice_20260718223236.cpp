...
void PulsarSynthVoice::spawnGrain() {
  ...
  float caPitchOffset = (0.9f + 0.2f * (static_cast<float>(active) / 8.0f)) * perlinMod;  // [0.9, 1.1]*perlin
  float startPhaseOffset = static_cast<float>(active) * perlinMod / 8.0f;                 // [0, 1]* perlinMod
  float ampRandom = (2.0f * static_cast<float>(active) / 8.0f / 2.0f - 0.5f) * perlinMod; // [-0.5, 0.5]*perlin
  ...
  int delaySamples = static_cast<int>((currentStateDurationSampleNum / 100) * (static_cast<float>(active) / 8.0f));
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    //  completeWindow * getPulseFadeGain() * windowInCurrentStage  * g.amp * amLfo
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  return 0.5f + getFmLfoValueAtPhase(phase) * depth * 1.5f + (2.0f * getCellActiveCount() * 0.1 / 8.0f - 0.1f); // [0.5, 2] +/- [0, 0.1]
}

float PulsarSynthVoice::calcAmLfoInterpolation(float phase, float depth) {
  return 0.2f + getAmLfoValueAtPhase(phase) * depth * 0.8 + (2.0f * getCellActiveCount() * 0.1 / 8.0f - 0.1f); // [0.2, 1] +/- [0, 0.1]
}
