...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  // double lfoAmount = getCurrentPan(envIndex / 2048.0f, commonVoiceSate->panDepthParam->load());
  // maskPass = r < lfoAmount ? false : maskPass;

  if (maskPass) {
    // double fmEnvelope1 = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    // lfoPhase += lfoPhaseInc;
    // if (lfoPhase >= 1.0)
    //   lfoPhase -= 1.0;
    // double fmEnvelope2 = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    // lfoPhase += lfoPhaseInc;
    // if (lfoPhase >= 1.0)
    //   lfoPhase -= 1.0;
    // double fmEnvelope3 = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    // lfoPhase += lfoPhaseInc;
    // if (lfoPhase >= 1.0)
    //   lfoPhase -= 1.0;
    // double fmEnvelope4 = calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
    // lfoPhase += lfoPhaseInc;
    // if (lfoPhase >= 1.0)
    //   lfoPhase -= 1.0;

    spawnGrain(snapShot.fundamentalFreq, 1.0f);
    spawnGrain(snapShot.fundamentalFreq * 2.0f, 0.75f);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, 0.5f);
    spawnGrain(snapShot.fundamentalFreq * 4.0f, 0.25f);
    // spawnGrain(snapShot.fundamentalFreq * 5.0f, 0.45f);
    // spawnGrain(snapShot.fundamentalFreq * 6.0f, 0.35f);
    // spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.25f);
    // spawnGrain(snapShot.fundamentalFreq * 8.0f, 0.15f);
    // spawnGrain(snapShot.fundamentalFreq * 9.0f, 0.125f);
    // spawnGrain(snapShot.fundamentalFreq * 10.0f, 0.1f);
    spawnGrain(snapShot.fundamentalFreq * 11.0f, 0.05f);
  }
}
...
float PulsarSynthVoice::calSampleByState(bool passMaskFlag, bool existMask) {
  ...
  // 渲染路径：mask已在trigger处gate(maskGatedSpawn，不spawn即无声)，这里永远只求和活跃grain：
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor) {
  ...
  // double trainPhase = static_cast<double>(periodsDoneInTrain) / snapShot.trainDutyCycleLenParam;
  // double trainPhaseInc = static_cast<float>(baseFreq / (snapShot.trainDutyCycleLenParam * getSampleRate()));
  ...
  // 高PRF时衰减per-grain随机音高抖动：脉冲密集(>20Hz)融合成音高后，周期抖动会把线状谱抹成噪声谱(noise感)；低PRF时保留全部抖动作为有机感
  // double prf = baseFreq;
  // double jitterAtten = prf > 20.0 ? 20.0 / prf : 1.0;
  // caPitchOffset = 1.0 + (caPitchOffset - 1.0) * jitterAtten;
  double scanFreq = baseFreq * static_cast<double>(fmSlice);
  ...
  float driftPhase = perlinNoise.noise(static_cast<float>(perlinTime * 0.1 + 37.0));
  ...
  amp *= 1.0f + driftAmp * 0.1f; // ±10%音量呼吸感
  double cycleSamples = std::round(getSampleRate() / scanFreq);

  // 增加一点pitch抖动 [-1, 1]
  // double lfoAmount = std::sin(2.0f * juce::MathConstants<float>::pi * baseFreq * fmLatch) * amLatch;
  // 相位相干：grain准时在period边界触发、从phase 0开始——重叠时同相叠加变厚而不是echo；
  // 之前的随机delay(最多~8ms)和随机起始相位会把重叠grain变成错开的delay tap，产生echo感
  int delaySamples = 0;
  double startPhase = 0;
  delaySamples = r < 0.3 ? active / 8.0f + driftPhase : driftPhase;
  startPhase = r > 0.3 ? active / 8.0f + driftPhase : driftPhase;
  // envelope dilation(GrainBuf: dur = dilation/ffreq, rate不变)：
  ...
  double phaseInc = cycleSamples > 0.0 ? 1.0 / cycleSamples : 0.0;
  ...
  // 下一个train从0重新扫
  // float wtPos = trainPhase;
  // float wtPosInc = trainPhaseInc;
  ...
  // 叠加慢速perlin漂移(±3%)：扫描轨迹在标称位置附近缓慢游走，帧morph多一层自动微变化
  float wtPos = juce::jlimit(0.0f, 1.0f, static_cast<float>(envPhase));
  float wtPosInc = static_cast<float>(baseFreq / (2048.0f * getSampleRate()));

  // 调制器速率跟随频率(envM = ffreq * envMul * 2048/sr)——LFO频率 = grain波形频率(ffreq)，
  ...
  lfoPhaseInc = cycleSamples > 0.0 ? 1.0f / cycleSamples : 0.0f;
  ...
  if (slot >= 0) {
    waveformGrains[slot] = {startPhase, phaseInc, lfoPhase, lfoPhaseInc, true, playbackSamples, playbackSamples, delaySamples, 1.0f, 0, amp, static_cast<float>(fmLatch), wtPos, wtPosInc};
  ...
  // randomly remove a grian
  // if (r < 0.3 && count > 5) {
  //   for (int i = 0; i < MAX_WAVEFORM_GRAINS; ++i) {
  //     int idx = (waveformGrainWriteIdx + i) % MAX_WAVEFORM_GRAINS;
  //     if (waveformGrains[idx].active) {
  //       waveformGrains[idx].active = false;
  //       break;
  //     }
  //   }
  // }
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    // wavetable帧位置逐sample连续滑动；ping-pong反弹代替wrap：长grain(大cluster)中途到达边界时
    // 帧位置往回滑而不是从末帧跳回首帧——wrap的音色突变会发生在窗值最大的中段，产生毛刺
    // scanning frames from 0 to len - 1 then scanning reversely
    g.wtPos += g.wtPosInc;
    if (g.wtPos >= 1.0f) {
      g.wtPos = 2.0f - g.wtPos;
      g.wtPosInc = -g.wtPosInc;
      // 随机frames
      if (r < 0.7) {
        // 随机frames
        static std::mt19937 rng{std::random_device{}()};
        std::shuffle(commonVoiceSate->pgWavetable.begin(), commonVoiceSate->pgWavetable.end(), rng);
      }
    } else if (g.wtPos < 0.0f) {
      g.wtPos = -g.wtPos;
      g.wtPosInc = -g.wtPosInc;
    }

    // 寿命窗从整段hann改为Tukey(首尾10%淡化+中段平顶)：hann×ADSR把每个grain挤成窄鼓包，
    // 重叠求和在grain中心之间出现深谷(PRF处泵动)；平顶窗让重叠部分求和近似恒定，
    // 包络形状交给用户的per-grain ADSR(自带attack/release归零，防click不受影响)
    double completeWindow = cycleEdgeWindow(static_cast<float>(g.windowPhase), 0.1f);
    // 周期内只做首尾短淡化(声明wrap处防click)，不再整周期hann斩波——overlap叠加更平滑
    double completeWindow2 = cycleEdgeWindow(static_cast<float>(g.phase));
    // per-grain ADSR窗与hann窗采样同一时刻的windowPhase(自增前)，避免错位一个sample；
    ...
    double fmEnvelope = calcFmLfoInterpolation(static_cast<float>(g.phase), commonVoiceSate->fmLfoDepthParam->load());
    ...
    grainSum += raw * g.amp * pulsaretEnvelope * completeWindow * completeWindow2 * adsrWindow;
    // 相位只按Latch的rate推进：逐sample用perlin/LFO/ratio晃相位会让各grain音高各自漂移、
    // 互相抵消(washy)；有机感由spawn时的fm/am Latch和包络负责
    g.phase += g.phaseInc * fmEnvelope;
    if (g.phase >= 1.0f) {
      g.phase -= 1;
    }
