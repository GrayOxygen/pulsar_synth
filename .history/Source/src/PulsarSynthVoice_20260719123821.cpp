...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * completeWindow * getPulseFadeGain() * windowInCurrentStage * amLfo * g.scale; // am modulation
    // grainSum += raw * completeWindow * getPulseFadeGain() * windowInCurrentStage ; // am modulation
    grainSum = juce::dsp::FastMathApproximations::tanh(grainSum) * g.amp;
    activeCount++;
