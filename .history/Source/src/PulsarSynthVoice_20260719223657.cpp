...
float PulsarSynthVoice::calcFmEnvelopeInterpolation(float phase, float depth) { return juce::jmap(getFmEnvelopeValueAtPhase(phase), 0.0f, 1.0f, -1.0f, 1.0f) * depth; }
...
float PulsarSynthVoice::calcFmLfoInterpolation(float phase, float depth) {
  float env = getFmLfoValueAtPhase(phase); // 0~1
  float semitone = juce::jmap(env, 0.0f, 1.0f, -1.0f, 1.0f);
...
float PulsarSynthVoice::getWaveformEnvelopeValueAtPhase(float phase) const {
  ...
  // 波形闭合crossfade：手绘波形首尾值可能不相等，phase wrap(1->0)时会产生台阶(咔哒声)。
  // 在最后5%相位区间把尾部值渐变到表头值，保证wrap处严格连续；对首尾本就相接的波形(如sine)几乎无影响
  constexpr float crossfadeRatio = 0.05f;
  float p = phase - std::floor(phase);
  if (p > 1.0f - crossfadeRatio) {
    float t = (p - (1.0f - crossfadeRatio)) / crossfadeRatio;
    float head = commonVoiceSate->pgWaveformEnvelopeData[0] * scale;
    v = v * (1.0f - t) + head * t;
  }
  return v;
}
