...
void PulsarSynthVoice::spawnGrain() {
  ...
  waveformGrains[waveformGrainWriteIdx] = {0, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, 0, startPhaseOffset, phaseInc, wtPos};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double amLfo = calcAmLfoInterpolation(static_cast<float>(g.lfoPhaseInc), commonVoiceSate->amLfoDepthParam->load());
