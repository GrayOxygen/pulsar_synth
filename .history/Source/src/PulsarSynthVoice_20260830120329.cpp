...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    spawnGrain(snapShot.fundamentalFreq * 2.0f, amLatch * r, 0.75f, false, triangle);
    spawnGrain(snapShot.fundamentalFreq * 3.0f, 1.0f * r, 0.75f, false, sine);
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod) {
  ...
  lfoPhaseInc = getSampleRate() > 0.0 ? scanFreq * (1.0f + rateMod) / getSampleRate() : 0.0;
