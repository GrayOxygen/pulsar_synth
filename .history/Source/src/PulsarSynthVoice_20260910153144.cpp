...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    if (harmonics > 1.0e-3f) {
      spawnGrain(snapShot.fundamentalFreq * 2.0, harmonics / 2.0, 1.0f * fmLatch, false, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 3.0, harmonics / 3.0, 1.0f * r, false, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 4.0, harmonics / 4.0, 1.0f * (r + fmLatch) / 2.0f, false, 1.0f);
    }
