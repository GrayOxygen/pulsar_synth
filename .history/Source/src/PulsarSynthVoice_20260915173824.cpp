...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  double scanFreq = baseFreq / static_cast<int>(std::round(1.0f + fmSlice * 100.0f));
  ...
  // double durCycles = static_cast<int>(std::round(1.0f + snapShot.dutyCycleCluster * 99.0f));
  if (durCycles <= 0 || cycleSamples <= 0) {
    return;
  }
  // 取整到整数个波形周期
  int playbackSamples = static_cast<int>(std::round(cycleSamples * durCycles));
