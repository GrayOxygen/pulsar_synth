...
void GranularEngine::renderSample(double sampleRate, float &outL, float &outR) {
  ...
  for (auto &g : grains) {
    ...
    const double panAngle = (static_cast<double>(g.pan) + 1.0) * 0.25 * juce::MathConstants<double>::pi;
    grainSumL += contribution * std::cos(panAngle) * juce::MathConstants<double>::sqrt2;
    grainSumR += contribution * std::sin(panAngle) * juce::MathConstants<double>::sqrt2;
  ...
  const double norm = 1.0 / std::sqrt(static_cast<double>(smoothedActiveCount));
