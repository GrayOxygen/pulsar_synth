...
void PulsarSynthVoice::saveSnapShot() {
  ...
  snapShot.dutyCycleCluster =
      getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate, commonVoiceState->kEnvIndexWrapParam.load()), commonVoiceSate->dutyCycleClusterDepthParam->load());
...
void PulsarSynthVoice::refreshPulsarInSnapShot() {
  ...
  snapShot.dutyCycleRatio = 1.0f;
  snapShot.dutyCycleCluster =
      getCurrentDutyCycleCluster(envTimelinePhase(envIndex, kClusterEnvTimelineRate, commonVoiceSate->kEnvIndexWrapParam.load()), commonVoiceSate->dutyCycleClusterDepthParam->load());
  snapShot.dutyCycleTime = snapShot.dutyCycleRatio * snapShot.pulsarPeriodTime;
  snapShot.pulsarSilenceTime = (1 - snapShot.dutyCycleRatio) * snapShot.pulsarPeriodTime;
  snapShot.pulsarFreq = 1.0 / snapShot.dutyCycleTime;
...
void PulsarSynthVoice::mappingParams(const juce::AudioProcessorValueTreeState &apvts) {
  ...
  commonVoiceSate->unisonWidthParam = apvts.getRawParameterValue(why::ParameterID::waveScanSpeed);
...
void PulsarSynthVoice::mappingOneParam(const juce::AudioProcessorValueTreeState &apvts, juce::String parameterID, float newValue) {
  ...
  if (parameterID == why::ParameterID::waveScanSpeed && commonVoiceSate->unisonWidthParam != nullptr) {
...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  const float pan = getCurrentPan(envTimelinePhase(envIndex, kPanEnvTimelineRate, commonVoiceSate->kEnvIndexWrapParam.load()), commonVoiceSate->panDepthParam->load());
  if (maskPass) {
...
void PulsarSynthVoice::spawnGrain(double baseFreq, double amFactor, double length, bool fund, double rateMod, double extraDetuneCents, float pan) {
  ...
  float amp = calcAmEnvelopeInterpolation(envTimelinePhase(envIndex, kAmEnvTimelineRate, commonVoiceSate->kEnvIndexWrapParam.load()), commonVoiceSate->amEnvDepthParam->load());
  double fmSlice = calcFmEnvelopeInterpolation(envTimelinePhase(envIndex, kFmEnvTimelineRate, commonVoiceSate->kEnvIndexWrapParam.load()), commonVoiceSate->fmEnvelopeDepthParam->load());
