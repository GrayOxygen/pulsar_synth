...
void PulsarSynthVoice::spawnGrain() {
  ...
  double cycles = std::max(1, static_cast<int>(std::round(snapShot.dutyCycleCluster)));                                         // grain内包含的完整周期数(取整)
  ...
  // 首尾天然同点，不再需要相位延续
  waveformGrains[waveformGrainWriteIdx] = {0.0, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, wtPos, wtPosInc};
