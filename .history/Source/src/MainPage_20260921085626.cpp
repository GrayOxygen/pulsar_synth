...
void MainPage::connectUIAndAudioParameter() {
  ...
  waveScanSpeedAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(processorRef.apvts, why::ParameterID::waveScanSpeed, waveScanSpeedSlider);
  envelopeScanSpeedAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(processorRef.apvts, why::ParameterID::envelopeScanSpeed, envelopeScanSpeedSlider);
...
void MainPage::makeVisible() {
  ...
  addAndMakeVisible(waveScanSpeedLabel);
  addAndMakeVisible(envelopeScanSpeedLabel);
  addAndMakeVisible(harmonicsSlider);
  addAndMakeVisible(unisonDetuneSlider);
  addAndMakeVisible(waveScanSpeedSlider);
  addAndMakeVisible(envelopeScanSpeedSlider);
}
...
void MainPage::topFlexBox(juce::FlexBox &flexBoxTop, std::shared_ptr<juce::FlexBox> trainLenFlexBox, std::shared_ptr<juce::FlexBox> trainDutyCycleFlexBox,
                          std::shared_ptr<juce::FlexBox> trainSilenceLenFlexBox, std::shared_ptr<juce::FlexBox> gainFlexBox) {
  ...
  gainFlexBox->flexDirection = juce::FlexBox::Direction::row;
  gainFlexBox->justifyContent = juce::FlexBox::JustifyContent::flexStart;
  gainFlexBox->items.add(juce::FlexItem(outputGainLabel).withFlex(1).withMaxWidth(100));
  gainFlexBox->items.add(juce::FlexItem(outputGainSlider).withFlex(2.5));
  // gainFlexBox->items.add(juce::FlexItem().withFlex(0.1));
  // gainFlexBox->items.add(juce::FlexItem(bpmSlider).withFlex(1));
  flexBoxTop.items.add(juce::FlexItem(*gainFlexBox).withFlex(1.0f));
}
void MainPage::midFlexBox(juce::FlexBox &midFlexBox, std::shared_ptr<juce::FlexBox> maskOptionFlexBox, std::shared_ptr<juce::FlexBox> burstMaskFlexBox,
  ...
  const int heightLow = 50.0f;
  const int heightHigh = 100.0f;
  //==============================================================
  // Outer row
  //==============================================================

  midFlexBox.flexDirection = juce::FlexBox::Direction::row;
  midFlexBox.justifyContent = juce::FlexBox::JustifyContent::flexStart;
  midFlexBox.alignItems = juce::FlexBox::AlignItems::stretch;
  ...
  maskOptionFlexBox->alignItems = juce::FlexBox::AlignItems::center;

  maskOptionFlexBox->items.add(juce::FlexItem(maskComboBoxLabel).withWidth(80.0f).withHeight(heightLow));

  maskOptionFlexBox->items.add(juce::FlexItem(maskOptionComboBox).withFlex(1.0f).withHeight(heightHigh));
  ...
  burstMaskFlexBox->alignItems = juce::FlexBox::AlignItems::center;

  burstMaskFlexBox->items.add(juce::FlexItem(burstMaskLabel).withWidth(80.0f).withHeight(heightLow));

  burstMaskFlexBox->items.add(juce::FlexItem(burstMaskTextEditor).withFlex(1.0f).withHeight(heightHigh));
  ...
  euclidStepFlexBox->alignItems = juce::FlexBox::AlignItems::center;
  euclidStepFlexBox->items.add(juce::FlexItem(euclidStepLabel).withWidth(80.0f).withHeight(heightLow));
  euclidStepFlexBox->items.add(juce::FlexItem(euclidStepSlider).withFlex(1.0f).withHeight(heightHigh));
  ...
  euclidHitFlexBox->alignItems = juce::FlexBox::AlignItems::center;
  euclidHitFlexBox->items.add(juce::FlexItem(euclidHitLabel).withWidth(80.0f).withHeight(heightLow));
  euclidHitFlexBox->items.add(juce::FlexItem(euclidHitSlider).withFlex(1.0f).withHeight(heightHigh));
  ...
  stochasticMaskFlexBox->alignItems = juce::FlexBox::AlignItems::center;

  stochasticMaskFlexBox->items.add(juce::FlexItem(stochasticMaskLabel).withWidth(80.0f).withHeight(heightLow));

  stochasticMaskFlexBox->items.add(juce::FlexItem(stochasticMaskTextEditor).withFlex(1.0f).withHeight(heightHigh));

  midFlexBox.items.add(juce::FlexItem(*stochasticMaskFlexBox).withFlex(0.9f));
}
...
void MainPage::resized() {
  ...
  // middle bottom
  juce::FlexBox middleBottomFlexBox;
  middleBottomFlexBox.flexDirection = juce::FlexBox::Direction::row;
  middleBottomFlexBox.justifyContent = juce::FlexBox::JustifyContent::flexStart;
  ...
  middleBottomFlexBox.items.add(juce::FlexItem(*adsrFlexBox).withFlex(1.0f));
  ...
  extendFlexBox->items.add(juce::FlexItem(harmonicLabel).withFlex(2).withMaxWidth(100).withMaxHeight(20));
  extendFlexBox->items.add(juce::FlexItem(harmonicsSlider).withFlex(0.5).withMaxWidth(100).withMaxHeight(80));
  extendFlexBox->items.add(juce::FlexItem(detuneLabel).withFlex(2).withMaxWidth(100).withMaxHeight(20));
  extendFlexBox->items.add(juce::FlexItem(unisonDetuneSlider).withFlex(0.5).withMaxWidth(100).withMaxHeight(80));
  extendFlexBox->items.add(juce::FlexItem(waveScanSpeedLabel).withFlex(2).withMaxWidth(100).withMaxHeight(20));
  extendFlexBox->items.add(juce::FlexItem(waveScanSpeedSlider).withFlex(1.0).withMaxWidth(100).withMaxHeight(80));
  extendFlexBox->items.add(juce::FlexItem(envelopeScanSpeedLabel).withFlex(1).withMaxWidth(100).withMaxHeight(20));
  extendFlexBox->items.add(juce::FlexItem(envelopeScanSpeedSlider).withFlex(1.0).withMaxWidth(100).withMaxHeight(80));
  middleBottomFlexBox.items.add(juce::FlexItem(*extendFlexBox).withFlex(1.0f));
  ...
  mainFlexBox.items.add(juce::FlexItem(topFlexBox).withFlex(1.0f).withMargin({20.0f, 20.0f, 0.0f, 20.0f})); // 上右下左
  mainFlexBox.items.add(juce::FlexItem(midFlexBox).withFlex(0.3f).withMargin({50.0f, 20.0f, 0.0f, 20.0f}));
  mainFlexBox.items.add(juce::FlexItem(middleBottomFlexBox).withFlex(1.0f).withMargin({50, 20, 0, 20}));
  mainFlexBox.items.add(juce::FlexItem(pgWaveformRowFlexBox).withFlex(3.0f).withMargin({10, 10, 20, 20}));
...
void MainPage::setUIStyle() {
  ...
  bpmEnvelopeLabel.setText("BPM Envelope", juce::dontSendNotification);
  bpmEnvelopeCanvas.setYAxisRange(30.0f, 210.0f);
  ...
  bpmYMinSlider.setRange(30.0, 120.0, 1.0);
  ...
  bpmYMaxSlider.setRange(120.0, 210.0, 1.0);
  bpmYMaxSlider.setValue(210.0, juce::dontSendNotification);
  ...
  harmonicLabel.setText("Harmonic", juce::dontSendNotification);
  detuneLabel.setText("Detune", juce::dontSendNotification);
  waveScanSpeedLabel.setText("Wave Scan Speed", juce::dontSendNotification);
  envelopeScanSpeedLabel.setText("Envelope Scan Speed", juce::dontSendNotification);
  ...
  setupExtendSlider(waveScanSpeedSlider, "Wave Scan Speed: spread of the waveform files");
  setupExtendSlider(envelopeScanSpeedSlider, "Envelope Scan Speed");

  // ========================== Pg Waveform 包络绘制控件样式 ==========================
  pgWaveformEnvelopeLabel.setText("Pg Waveform (max 32 files)", juce::dontSendNotification);
  ...
  pgWaveformLoadFileButton.onClick = [this] {
    ...
    chooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles | juce::FileBrowserComponent::canSelectMultipleItems,
                         [this, chooser](const juce::FileChooser &fc) {
                           ...
                           // wavetable scanning：每个文件均分为FRAMES_PER_FILE段，每段取连续WAVETABLE_SIZE样本为一帧，
                           // 按文件顺序拼接成帧序列；scanning在文件内morph，并平滑过渡到下一个文件。
                           constexpr int targetSize = CommonVoiceState::WAVETABLE_SIZE;
                           std::vector<std::array<float, targetSize>> frames;

                           // scan all files
                           for (const auto &file : results) {
                             if (static_cast<int>(frames.size()) >= CommonVoiceState::WAVETABLE_FRAMES)
                               break;
                             if (!file.existsAsFile())
                               continue;
                             ...
                             juce::AudioBuffer<float> srcBuffer(1, readSamples);
                             reader->read(&srcBuffer, 0, srcBuffer.getNumSamples(), 0, true, false);
                             // the buffer of single file
                             const float *src = srcBuffer.getReadPointer(0);
                             int srcLen = srcBuffer.getNumSamples();
                             int numFileFrames = CommonVoiceState::FRAMES_PER_FILE;
                             if (srcLen < targetSize * 2)
                               numFileFrames = 1; // 文件太短，退化为单帧
                             numFileFrames = std::min(numFileFrames, CommonVoiceState::WAVETABLE_FRAMES - static_cast<int>(frames.size()));

                             // 连续切片保留源素材真实的频谱内容(音色/谐波结构)，切片起点沿文件均匀分布，scanning时扫过样本的真实音色轨迹
                             for (int f = 0; f < numFileFrames; ++f) {
                               std::array<float, targetSize> frame;
                               // length is longer than one frame (WAVETABLE_SIZE samples)
                               if (srcLen >= targetSize) {
                                 int maxStart = srcLen - targetSize;
                                 int frameStart = numFileFrames > 1 ? static_cast<int>(static_cast<int64>(maxStart) * f / (numFileFrames - 1)) : 0;
                           ...
                           juce::MessageManager::callAsync([this, frames] {
                             ...
                             // canvas只展示第一帧(2048点下采样到画布点数)；notify=false，避免canvas listener把wavetable覆盖回单帧
                             pgWaveformEnvelopeCanvas.setEnvelopeData(PulsarSynth::resampleLinear<CommonVoiceState::WAVETABLE_SIZE, EnvelopeCanvas::ENVELOPE_SIZE>(frames[0]), false);
                           });
                         });
  };
  // ===============================================================================================
