...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    float f = snapShot.fundamentalFreq * std::pow(2.0, static_cast<double>(driftPitch) * 12.0 / 1200.0);
    ...
    //            1.0f                                                                                       //
    // );
    spawnGrain(snapShot.fundamentalFreq, 1.0f, (fmLatch == 0 ? 0 : 1.0f / fmLatch), false, (std::cos(amLatch * r) * std::cos(amLatch * r) + std::sin(amLatch) * std::sin(amLatch)));

    // spawnGrain(snapShot.fundamentalFreq * 2.0f, amLatch, 1.0f * fmLatch, false, std::sin(amLatch) * std::cos(amLatch * r));
    // spawnGrain(snapShot.fundamentalFreq * 3.0f, amLatch, 0.75f * fmLatch, false, std::cos(amLatch * r));
    // spawnGrain(snapShot.fundamentalFreq * 4.0f, amLatch * r, 0.5f * fmLatch, false, std::sin(amLatch) * r);
  }
}
