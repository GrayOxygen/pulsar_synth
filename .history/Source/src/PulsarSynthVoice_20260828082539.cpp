...
void PulsarSynthVoice::maskGatedSpawn() {
  ...
  if (maskPass) {
    ...
    if (r % 5 == 4) {
      spawnGrain(snapShot.fundamentalFreq, 1.0f);
      spawnGrain(snapShot.fundamentalFreq * 7.0f, 0.75f);
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    double pulsaretEnvelope = calcAmLfoInterpolation(static_cast<float>(g.phase), commonVoiceSate->amLfoDepthParam->load()) * //
                              calcAmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->amLfoDepthParam->load());
    double fmEnvelope = calcFmLfoInterpolation(static_cast<float>(g.phase), commonVoiceSate->fmLfoDepthParam->load()) * //
                        calcFmLfoInterpolation(static_cast<float>(lfoPhase), commonVoiceSate->fmLfoDepthParam->load());
