...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    if (g.remainSamples <= 1)
      completeWindow = 0.0f;
    grainSum += raw * completeWindow * gaussianEnvelope(static_cast<float>(currentStateDurationSampleNum - g.remainSamples) / static_cast<float>(currentStateDurationSampleNum - 1), 0.5) * g.amp *
                amLfo; // am modulation
    activeCount++;
