...
void PulsarSynthVoice::spawnGrain() {
  ...
  // int samples = static_cast<int>(currentStateDurationSampleNum * snapShot.dutyCycleCluster * (1.0f + 1.0f * active / 8.0f));
  // int delaySamples = static_cast<int>((currentStateDurationSampleNum / 16.0f) * (1.0f * active > 5 ? active : 0));
  // float amp = calcAmEnvelopeInterpolation(envPhase, commonVoiceSate->amEnvDepthParam->load()) * (1 + 2 * static_cast<float>(active) / 8.0f / 10.0f - 0.1f);
  ...
  startPhaseOffset = static_cast<double>(waveformGrains[prevGrainIdx].phase) * 0.5f * static_cast<float>(active) / 8.0f;
  
  // wavetable scanning：扫描位置由train相位决定，后续每个pulsar依次扫到不同的波形帧
  float wtPos = static_cast<float>(envPhase);

  waveformGrains[waveformGrainWriteIdx] = {startPhaseOffset, phaseInc, true, samples, samples, delaySamples, scale, 0, amp, 0, startPhaseOffset, phaseInc, wtPos};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    grainSum += raw * completeWindow * g.amp * amLfo * g.scale; // am modulation
