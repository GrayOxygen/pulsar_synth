...
void PulsarSynthVoice::spawnGrain() {
  ...
  float caPitchOffset = (0.9f + 0.3f * (static_cast<float>(active) / 8.0f)) * perlinMod; // [0.9, 1.2]*perlin
  float startPhaseOffset = static_cast<float>(active) / 8.0f * perlinMod;                  // [0, 1]*perlin
  float ampRandom = (2.0f * static_cast<float>(active) / 8.0f / 10.0f - 0.1f) * perlinMod; // [-0.1, 0.1]*perlin
  ...
  waveformGrains[waveformGrainWriteIdx] = {startPhaseOffset, phaseInc, true, samples, samples, delaySamples, 0, amp};
