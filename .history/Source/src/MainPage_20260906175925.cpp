...
void MainPage::connectUIAndAudioParameter() {
  ...
  // async grain cloud
  cloudDensityAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(processorRef.apvts, why::ParameterID::cloudDensity, cloudDensitySlider);
  cloudLevelAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(processorRef.apvts, why::ParameterID::cloudLevel, cloudLevelSlider);
  cloudPitchSpreadAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(processorRef.apvts, why::ParameterID::cloudPitchSpread, cloudPitchSpreadSlider);
...
void MainPage::makeVisible() {
  ...
  addAndMakeVisible(cloudDensitySlider);
  addAndMakeVisible(cloudLevelSlider);
  addAndMakeVisible(cloudPitchSpreadSlider);
}
...
void MainPage::midFlexBox(juce::FlexBox &midFlexBox, std::shared_ptr<juce::FlexBox> triggerFlexBox, std::shared_ptr<juce::FlexBox> maskOptionFlexBox, std::shared_ptr<juce::FlexBox> burstMaskFlexBox,
                          ...
                          std::shared_ptr<juce::FlexBox> adsrFlexBox, std::shared_ptr<juce::FlexBox> cloudFlexBox) {
  ...
  // async grain cloud: ADSR后同一行的3个旋钮(密度/电平/音高散布)
  cloudFlexBox->flexDirection = juce::FlexBox::Direction::row;
  cloudFlexBox->items.add(juce::FlexItem(cloudLabel).withFlex(1).withMaxWidth(60).withMaxHeight(20));
  cloudFlexBox->items.add(juce::FlexItem(cloudDensitySlider).withFlex(1.0).withMaxWidth(100).withMaxHeight(80));
  cloudFlexBox->items.add(juce::FlexItem(cloudLevelSlider).withFlex(1.0).withMaxWidth(100).withMaxHeight(80));
  cloudFlexBox->items.add(juce::FlexItem(cloudPitchSpreadSlider).withFlex(1.0).withMaxWidth(100).withMaxHeight(80));
  midFlexBox.items.add(juce::FlexItem(*cloudFlexBox).withFlex(0.9f));
...
void MainPage::resized() {
  ...
  std::shared_ptr<juce::FlexBox> cloudFlexBox = std::make_shared<juce::FlexBox>();
  this->midFlexBox(midFlexBox, row10, row11, row12, row13, row14, row15, adsrFlexBox, cloudFlexBox);
...
void MainPage::setUIStyle() {
  ...
  // ========================== async grain cloud控件样式 ==========================
  cloudLabel.setText("Cloud", juce::dontSendNotification);
  auto setupCloudSlider = [](juce::Slider &s, const juce::String &tip) {
    s.setSliderStyle(juce::Slider::RotaryVerticalDrag);
    s.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 40, 16);
    s.setTooltip(tip);
  };
  setupCloudSlider(cloudDensitySlider, "Cloud density: async grains per second (0 = off)");
  setupCloudSlider(cloudLevelSlider, "Cloud level: amplitude of async cloud grains");
  setupCloudSlider(cloudPitchSpreadSlider, "Cloud pitch spread: random detune range (semitones)");
