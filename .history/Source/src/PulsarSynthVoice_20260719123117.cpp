...
void PulsarSynthVoice::spawnGrain() {
  ...
  waveformGrains[waveformGrainWriteIdx] = {0, phaseInc, true, samples, samples, scale, 0, amp};
...
float PulsarSynthVoice::calcActualPulse() {
  ...
  for (auto &g : waveformGrains) {
    ...
    // grainSum += raw * completeWindow * getPulseFadeGain() * windowInCurrentStage * g.amp * amLfo * g.scale; // am modulation
    grainSum += raw * completeWindow * getPulseFadeGain() * windowInCurrentStage * g.amp; // am modulation
    grainSum = juce::dsp::FastMathApproximations::tanh(pulseReadPtr[i]) * outputGain;
