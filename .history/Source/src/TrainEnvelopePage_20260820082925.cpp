...
void TrainEnvelopePage::resized() {
  ...
  pulsarLengthRowFlexBox.items.add(juce::FlexItem(panDepthEnvelopeFlexBox).withFlex(0.3f));
  ...
  pulsarLengthRowFlexBox.items.add(juce::FlexItem(panEnvelopeFlexBox).withFlex(1.0f));
...
void TrainEnvelopePage::setUIStyle() {
  ...
  dutyCycleClusterEnvelopeScaleSlider.setRange(0.0f, 32.0, 0.01);
...
void TrainEnvelopePage::initUITriggerEvent() {
  ...
  panEnvelopeScaleSlider.onValueChange = [this] {
    processorRef.getPulsarSynthEngine().executeCurSynthCallback(
        [&](std::shared_ptr<PulsarSynth> &synth) { synth->setPanEnvelopeScale(static_cast<float>(panEnvelopeScaleSlider.getValue())); });
  };
