...
void PulsarSynthVoice::spawnGrain() {
  ...
  double pulsarFreq = snapShot.pulsarFreq;
  double rate = fmSlice * fmLatch;
  // 得到最终cycle频率
  double cycleFreq = pulsarFreq * rate;
  // 限制最终cycle频率
  cycleFreq = std::clamp(cycleFreq, 20.0, 3000.0);
  // 再反推出真正使用的rate
  rate = cycleFreq / pulsarFreq;
  // 根据新的rate计算cycle长度
  double cycleSamples = grainBaseSamples / rate;
