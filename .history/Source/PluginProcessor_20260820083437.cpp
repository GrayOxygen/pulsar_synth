...
void AudioPluginAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock) {
  ...
  // DC blocker：一阶高通、5Hz截止。
  // 之前用30Hz二阶高通，低频段(低PRF的pulse train能量)会被相位扭曲并在每个pulse后产生下冲(undershoot)，
  // 听起来反而不干净；纯去DC只需要极低截止的一阶高通(REAPER的DC remover同理)
  // auto coefficients = FilterCoefs::makeFirstOrderHighPass(sampleRate, 20.0f);
  auto coefficients = FilterCoefs::makeHighPass(sampleRate, 5.0f,
                                                0.7071f); // Butterworth
...
juce::AudioProcessorValueTreeState::ParameterLayout AudioPluginAudioProcessor::createParameterLayout() {
  ...
  //   modulation depth
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::amEnvelopeDepth, 1), "AM Env Depth", 0.0, 1.0, 1.0));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::fmEnvelopeDepth, 1), "FM Env Depth", 0.1, 1.0, 1.0));
  ...
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::fmLfoDepth, 1), "FM LFO Depth", 0.1, 1.0, 1.0));
  ...
  // adsr
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::grainAdsrAttackSlider, 1), "Attack", 0.0, 1.0, 0.01));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::grainAdsrDecaySlider, 1), "Decay", 0.0, 1.0, 0.01));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::grainAdsrSustainSlider, 1), "Sustain", 0.0, 1.0, 0.01));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::grainAdsrReleaseSlider, 1), "Release", 0.0, 1.0, 0.01));
