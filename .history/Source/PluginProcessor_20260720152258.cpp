...
void AudioPluginAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock) {
  ...
  *highPassFilter.state = *juce::dsp::IIR::Coefficients<float>::makeHighPass(sampleRate, 20.0f, 2.0f);
  ...
  svfDCFilter.prepare(spec);

  // Set type to HighPass and frequency around 5.0 Hz to 10.0 Hz
  svfDCFilter.setType(juce::dsp::StateVariableFilter::Parameters<float>::Type::highPass);
  svfDCFilter.setCutoffFrequency(20.0f);
}
...
juce::AudioProcessorValueTreeState::ParameterLayout AudioPluginAudioProcessor::createParameterLayout() {
  ...
  params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID(why::ParameterID::fmEnvelopeDepth, 1), "FM Env Depth", 0.01, 1.0, 1.0));
