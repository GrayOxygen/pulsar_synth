...
public:
  ...
  void setCommonVoiceState(const std::shared_ptr<CommonVoiceState> &state) { commonVoiceState = state; }
  ...
  /**
   * Generic envelope lookup using linear interpolation over any table size
   * (调制包络=ENVELOPE_SIZE点，波表帧=WAVETABLE_SIZE点，各用各的分辨率)
   * @param data envelope data array
   * @param phase pulsaret phase (0.0 - 1.0)
   * @return interpolated envelope value
   */
  template <size_t S> static float getEnvelopeValueAtPhase(const std::array<float, S> &data, float phase, float scale = 1.0f) {
    constexpr int size = static_cast<int>(S);
    phase = std::fmod(std::abs(phase), 1.0f);
    float indexF = phase * (size - 1);
    int index = static_cast<int>(indexF);
    float frac = indexF - index;
    float val1 = data[static_cast<size_t>(std::min(index, size - 1))] * scale;
    float val2 = data[static_cast<size_t>(std::min(index + 1, size - 1))] * scale;
    return val1 + frac * (val2 - val1);
  }

  float calcFmLfoInterpolation(float phase, float depth);
...
private:
  ...
  std::shared_ptr<CommonVoiceState> commonVoiceState;
};
