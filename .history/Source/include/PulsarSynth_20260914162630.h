...
public:
  ...
  /**
   * Set BPM envelope data (44100 samples)
  ...
  /**
   * Set AM envelope data (44100 samples)
   */
  ...
  /**
   * Set FM envelope data (44100 samples)
   */
  ...
  /**
   * Set Duty Cycle Ratio envelope data (44100 samples)
   */
  ...
  /**
   * Set Duty Cycle Cluster envelope data (44100 samples)
   */
  ...
  static float calcHannWeightedMean(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
  ...
  void setPgWaveformEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        ...
        state->pgWavetable[0] = data;
        state->pgWavetableDcOffset[0] = calcHannWeightedMean(data);
        state->pgWavetableNumFrames = 1;
      }
    }
  }

  // wavetable scanning：加载多帧波表，每个pulsar根据train相位扫描到不同的帧
  void setPgWavetableData(const std::vector<std::array<float, EnvelopeCanvas::ENVELOPE_SIZE>> &frames) {
    ...
    if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
      ...
      state->pgWaveformEnvelopeData = frames[0]; // UI展示第一帧
