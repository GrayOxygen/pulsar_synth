...
public:
  ...
  // duty cycle ratio 包络数据 - 512 个 samples
  ...
  // duty cycle Cluster 包络数据 - 512 个 samples
  ...
  // Pg Waveform 包络数据 - 512 个 samples，Y轴范围 [-1, 1]，用户绘制的一个周期波形(仅供UI展示/同步，声音引擎读pgWavetable)
  ...
  static constexpr int WAVETABLE_SIZE = 512;
  ...
  // BPM 包络数据 - 512 个 samples，latch方式推进：每个stage走完一次，推进到下一个点
