...
public:
  ...
  /**
   * Set BPM envelope data (2048 samples)
  ...
  /**
   * 勾选后每次开启新train（mask走完一帧）时自动重新随机生成stochastic mask
   */
  void setAutoRandomStochasticMask(bool autoRandom) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceSate()->autoRandomStochasticMask.store(autoRandom);
      }
    }
  }
