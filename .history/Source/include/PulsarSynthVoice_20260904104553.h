...
private:
  ...
  //===========================waveform grain pool===========================
  // 池容量需 ≥ 最大重叠数×每trigger的spawn路数(基频+3路谐波=4)：
  // 16个slot在cluster≥4时就饱和，fundamental被丢弃→COLA阶梯缺孔→高发射率下周期性cut/click
...
public:
  ...
  void spawnGrain(double baseFreq, double amFactor = 1.0f, double length = 1.0f, bool fund = true, double rateMod = 1.0);
  ...
  float getWaveformEnvelopeValueAtPhase(float phase, float wtPos);
