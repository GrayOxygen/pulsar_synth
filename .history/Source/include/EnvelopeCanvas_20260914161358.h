...
public:
  ...
  static constexpr int ENVELOPE_SIZE = why::sampleRate;
  ...
  // 包络数据访问
  void setEnvelopeData(const std::array<float, ENVELOPE_SIZE> &data, bool notify = true);
  std::array<float, ENVELOPE_SIZE> getEnvelopeData() const;
  float getEnvelopeValueAtPhase(float phase) const; // phase: 0.0 - 1.0
