...
/**
 * Multiple voices share a CommonVoiceState to represent the same fields.
 * For example, burstMask and Voices have a one-to-many relationship
 */
class CommonVoiceState {
public:
  CommonVoiceState() {
    // 初始化包络数据为默认值 (1.0 = 无调制)
  ...
  static constexpr int WAVETABLE_SIZE = 9600;
  ...
  std::atomic<float> bpmEnvelopeYMax{210.0f};
  ...
  std::atomic<float> *envelopeScanSpeedParam;
};
