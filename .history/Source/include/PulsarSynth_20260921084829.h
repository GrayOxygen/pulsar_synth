...
public:
  ...
  /**
   * Set BPM envelope data (9600 samples)
  ...
  void setBpmEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->bpmEnvelopeData = data;
      }
    }
  }
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getBpmEnvelopeData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->bpmEnvelopeData;
      }
    }
  ...
  void setBpmEnvelopeYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->bpmEnvelopeYMin.store(yMin);
        voice->getCommonVoiceState()->bpmEnvelopeYMax.store(yMax);
      }
    }
  }
  ...
  /**
   * Update the burstMask of all voices, but the voices all point to the same CommonVoiceState,
  ...
  void refreshBurstMask(juce::String burstMask) {
    if (getNumVoices() > 0) {
      ...
      PulsarSynthVoice *pulsarVoice = dynamic_cast<PulsarSynthVoice *>(voice);
      pulsarVoice->getCommonVoiceState()->burstMask = burstMask.toStdString();
    }
  }
  ...
  std::string getStochasticMaskStr() {
    ...
    if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
      auto state = voice->getCommonVoiceState();
  ...
  void setUseAmpEnvelope(bool useEnvelope) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->useAmpEnvelope.store(useEnvelope);
      }
    }
  }
  ...
  void setAmpEnvelopeScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->ampEnvelopeScale.store(scale);
      }
    }
  }
  ...
  void setAmpEnvelopeYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->ampEnvelopeYMin.store(yMin);
        voice->getCommonVoiceState()->ampEnvelopeYMax.store(yMax);
      }
    }
  }

  /**
   * Set AM envelope data (9600 samples)
   */
  void setAmpEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->ampEnvelopeData = data;
      }
    }
  }
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getAmpEnvelopeData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->ampEnvelopeData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
  ...
  void setUseFmEnvelope(bool useEnvelope) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->useFmEnvelope.store(useEnvelope);
      }
    }
  }
  ...
  void setFmEnvelopeScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->fmEnvelopeScale.store(scale);
      }
    }
  }
  ...
  void setFmEnvelopeYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->fmEnvelopeYMin.store(yMin);
        voice->getCommonVoiceState()->fmEnvelopeYMax.store(yMax);
      }
    }
  }

  /**
   * Set FM envelope data (9600 samples)
   */
  void setFmEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->fmEnvelopeData = data;
      }
    }
  }
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getFmEnvelopeData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->fmEnvelopeData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
    defaultData.fill(1.0f);
  ...
  void setFmLfoScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->fmLfoScale.store(scale);
      }
    }
  }
  ...
  void setFmLfoYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->fmLfoYMin.store(yMin);
        voice->getCommonVoiceState()->fmLfoYMax.store(yMax);
      }
    }
  }

  /**
   * Set FM lfo data (9600 samples)
   */
  void setFmLfoData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->fmLfoData = data;
      }
    }
  }
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getFmLfoData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->fmLfoData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
  ...
  void setAmLfoScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->amLfoScale.store(scale);
      }
    }
  }
  ...
  void setAmLfoYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->amLfoYMin.store(yMin);
        voice->getCommonVoiceState()->amLfoYMax.store(yMax);
      }
    }
  }

  /**
   * Set AM lfo data (9600 samples)
   */
  void setAmLfoData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->amLfoData = data;
      }
    }
  }
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getAmLfoData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->amLfoData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
    defaultData.fill(0.0f);
  ...
  void setUsePanEnvelope(bool useEnvelope) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->usePanEnvelope.store(useEnvelope);
      }
    }
  }
  ...
  void setPanEnvelopeScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->panEnvelopeScale.store(scale);
      }
    }
  }
  ...
  void setPanEnvelopeYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->panEnvelopeYMin.store(yMin);
        voice->getCommonVoiceState()->panEnvelopeYMax.store(yMax);
      }
    }
  }

  /**
   * Set Duty Cycle Ratio envelope data (9600 samples)
   */
  void setPanEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->panEnvelopeData = data;
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getPanEnvelopeData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->panEnvelopeData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
  ...
  void setUseDutyCycleClusterEnvelope(bool useEnvelope) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->useDutyCycleClusterEnvelope.store(useEnvelope);
      }
    }
  }
  ...
  void setDutyCycleClusterEnvelopeScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->dutyCycleClusterEnvelopeScale.store(scale);
      }
    }
  }
  ...
  void setEnvelopeDilation(float dilation) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->envelopeDilation.store(dilation);
      }
    }
  }
  ...
  void setDutyCycleClusterEnvelopeYRange(float yMin, float yMax) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->dutyCycleClusterEnvelopeYMin.store(yMin);
        voice->getCommonVoiceState()->dutyCycleClusterEnvelopeYMax.store(yMax);
      }
    }
  }

  /**
   * Set Duty Cycle Cluster envelope data (9600 samples)
   */
  void setDutyCycleClusterEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->dutyCycleClusterEnvelopeData = data;
        // voice->setEnterNextTrain(true);
      }
    }
  }
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getDutyCycleClusterEnvelopeData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->dutyCycleClusterEnvelopeData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
    defaultData.fill(1.0f);
  ...
  void setUsePgWaveformEnvelope(bool useEnvelope) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->usePgWaveformEnvelope.store(useEnvelope);
      }
    }
  }
  ...
  void setPgWaveformEnvelopeScale(float scale) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        voice->getCommonVoiceState()->pgWaveformEnvelopeScale.store(scale);
      }
    }
  }
  ...
  void setPgWaveformEnvelopeData(const std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> &data) {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        ...
        state->pgWavetable[0] = resampleLinear<EnvelopeCanvas::ENVELOPE_SIZE, CommonVoiceState::WAVETABLE_SIZE>(data);
  ...
  void setPgWavetableData(const std::vector<std::array<float, CommonVoiceState::WAVETABLE_SIZE>> &frames) {
    if (frames.empty() || getNumVoices() == 0)
      return;
    if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
      auto state = voice->getCommonVoiceState();
      const int numFrames = std::min(static_cast<int>(frames.size()), CommonVoiceState::WAVETABLE_FRAMES);
      for (int f = 0; f < numFrames; ++f) {
        state->pgWavetable[f] = frames[f];
        state->pgWavetableDcOffset[f] = calcHannWeightedMean(frames[f]);
      }
      // UI展示第一帧(下采样到画布点数)
      state->pgWaveformEnvelopeData = resampleLinear<CommonVoiceState::WAVETABLE_SIZE, EnvelopeCanvas::ENVELOPE_SIZE>(frames[0]);
  ...
  std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> getPgWaveformEnvelopeData() const {
    if (getNumVoices() > 0) {
      if (auto *voice = dynamic_cast<PulsarSynthVoice *>(getVoice(0))) {
        return voice->getCommonVoiceState()->pgWaveformEnvelopeData;
      }
    }
    std::array<float, EnvelopeCanvas::ENVELOPE_SIZE> defaultData;
