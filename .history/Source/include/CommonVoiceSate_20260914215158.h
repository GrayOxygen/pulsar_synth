...
public:
  CommonVoiceSate() {
    ...
    // wavetable默认只有一帧(sine)：波表帧有自己的高分辨率(WAVETABLE_SIZE)，与UI包络的点数解耦
    for (int i = 0; i < WAVETABLE_SIZE; ++i) {
      float x = static_cast<float>(i) / (WAVETABLE_SIZE - 1);
      pgWavetable[0][static_cast<size_t>(i)] = std::sin(2.0f * 3.14159265f * x);
    }
  }
  ...
  // AM 包络数据（替代波形选择）- 96000 个 samples
  ...
  // FM 包络数据（替代波形选择）- 96000 个 samples
  ...
  // FM lfo数据（替代波形选择）- 96000 个 samples
  ...
  // AM lfo数据（替代波形选择）- 96000 个 samples
  ...
  // duty cycle ratio 包络数据 - 96000 个 samples
  ...
  // duty cycle Cluster 包络数据 - 96000 个 samples
  ...
  // Pg Waveform 包络数据 - 96000 个 samples，Y轴范围 [-1, 1]，用户绘制的一个周期波形(仅供UI展示/同步，声音引擎读pgWavetable)
  ...
  // 波表帧分辨率独立于UI包络点数(ENVELOPE_SIZE)：波形帧是真实音频内容，
  // 需要2048点保真；调制包络只是慢速控制曲线，96点足够
  static constexpr int WAVETABLE_SIZE = 96000;
  static constexpr int FRAMES_PER_FILE = 4;
  static constexpr int WAVETABLE_FRAMES = 32 * FRAMES_PER_FILE; //  WAVETABLE_FRAMES:最多frames数
  std::array<std::array<float, WAVETABLE_SIZE>, WAVETABLE_FRAMES> pgWavetable{};
  ...
  // BPM 包络数据 - 96000 个 samples，latch方式推进：每个stage走完一次，推进到下一个点
