...
│   │   ├── CommonVoiceState.h     # 多 Voice 共享参数与 Mask
...
| 项 | 路径 / 要求 |
|----|-------------|
| JUCE | 默认 `E:/env/juce-7.0.9600-windows/JUCE`（`-DPULSAR_JUCE_DIR=...` 可改） |
...
cmake -B build -G "MinGW Makefiles" -DPULSAR_JUCE_DIR="E:/env/juce-7.0.9600-windows/JUCE"
...
> JUCE 路径也可通过环境变量 `PULSAR_JUCE_DIR` 指定；未指定时默认 `E:/env/juce-7.0.9600-windows/JUCE`。
